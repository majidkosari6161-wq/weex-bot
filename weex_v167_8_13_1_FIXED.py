...
def get_open_demo_orders():
    """V167.8.11: try Demo symbol first, then contract symbol, with negative cache."""
    now = time.time()
    if (_OPEN_ORDERS_NEG_CACHE["reason"]
            and now - _OPEN_ORDERS_NEG_CACHE["ts"] < _OPEN_ORDERS_NEG_TTL):
        return []
    last_err = None
...
def reconcile_exchange_protection(position, item):
    """V167.8.13: verify exchange protection without inventing it."""
