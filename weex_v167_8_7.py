...
#   [V167.8.7-1] NEW: _get_effective_opened_at(position, item)
...
#   [V167.8.7-2] RECONCILIATION in _ensure_exit_state_for_position:
...
#   [V167.8.7-3] THESIS OBSERVATION now uses _get_effective_opened_at.
#   [V167.8.7-4] TP COMPRESSION now uses _get_effective_opened_at.
#   [V167.8.7-5] TIME EXIT now uses _get_effective_opened_at.
#
...
def _save_entry_atr_snapshot(symbol, side, created_ts_ms, entry_atr, entry_price, risk_distance):
    try:
        ...
        print(f"💾 V167.8.7 ENTRY_ATR SNAPSHOT SAVED: key={key} "
...
def _cleanup_entry_atr_snapshots(active_keys):
    try:
        ...
        os.replace(tmp, ENTRY_ATR_FILE)
        if to_remove:
            print(f"🧹 V167.8.7 ENTRY_ATR CLEANUP: removed {len(to_remove)} stale snapshot(s)")
    except Exception as e:
...
def _rebuild_entry_atr_from_history(df, created_ts):
    ...
    try:
        ...
        if pd.isna(atr_at_open):
            print(f"🚫 V167.8.7 HISTORY ATR: NaN at candle {entry_candle_ts}")
            return None
        atr_val = float(atr_at_open)
        if not np.isfinite(atr_val) or atr_val <= 0:
            print(f"🚫 V167.8.7 HISTORY ATR: invalid value {atr_val}")
            return None

        entry_close = float(x.loc[entry_candle_ts, "close"])
        if entry_close <= 0:
            print(f"🚫 V167.8.7 HISTORY ATR: entry_close={entry_close} invalid")
            return None
        atr_pct = atr_val / entry_close
        if atr_pct < ATR_SANITY_MIN_PCT or atr_pct > ATR_SANITY_MAX_PCT:
            print(f"🚫 V167.8.7 HISTORY ATR SANITY FAIL: "
        ...
        print(f"✅ V167.8.7 HISTORY ATR VERIFIED: candle={entry_candle_ts} "
...
def auth_headers(timestamp, signature):
    return {
        ...
        "User-Agent": "V167.8.7-Pro-Bot/1.0",
    }
...
def get_decayed_params():
    ...
    print(f"🔀 V167.8.7 DECAY: new params = {params}")
    return params


# ============================================================
# CIRCUIT BREAKER (V167.8.7: hardened)
# ============================================================
def get_circuit_breaker_state():
    path = Path(CIRCUIT_BREAKER_FILE)
    if not path.exists():
        return {"loss_streak": 0, "cooldown_until": None, "last_update": None}
    try:
...
def _reset_circuit_breaker_state(reason="UNKNOWN"):
    """V167.8.7: reset the circuit breaker to a clean state."""
    state = {
        "loss_streak": 0,
        "cooldown_until": None,
    ...
    try:
        ...
        print("🚨 CIRCUIT BREAKER RESET SAVE ERROR:", repr(e))
    return state


def update_circuit_breaker(is_loss, current_candle_time):
    """
    V167.8.7: caller MUST pass a real timestamp (pd.Timestamp or ISO string),
    not a price. Root-cause fix is in manage_position_exits.
    This function also guards against bad inputs defensively.
    """
    state = get_circuit_breaker_state()
    if is_loss:
        state["loss_streak"] = int(state.get("loss_streak", 0)) + 1
        if state["loss_streak"] >= CIRCUIT_MAX_LOSSES:
            # V167.8.7: normalize timestamp input defensively
            try:
                ts = pd.Timestamp(current_candle_time)
                if ts.tzinfo is None:
                    ts = ts.tz_localize("UTC")
                # Sanity: cooldown must be in the future (within reason).
                # If ts is a price-like number, pd.Timestamp will interpret it as
                # nanoseconds since 1970 → year 1970. Reject anything before 2020.
                if ts.year < 2020:
                    print(f"⚠️ V167.8.7 CIRCUIT BREAKER: bad timestamp input "
                          f"({current_candle_time!r} → {ts}); using now() instead")
                    ts = pd.Timestamp.now(tz="UTC")
            except Exception as e:
                print(f"⚠️ V167.8.7 CIRCUIT BREAKER: timestamp parse failed "
                      f"({current_candle_time!r}: {e}); using now() instead")
                ts = pd.Timestamp.now(tz="UTC")
...
def is_circuit_breaker_active(current_candle_time):
    """
    V167.8.7: hardened against tz-naive vs tz-aware comparison errors and
    legacy "1970-01-01" cooldown_until values (from V167.8.4/5's
    price-as-timestamp bug). Auto-resets invalid state.
    """
    state = get_circuit_breaker_state()
    if state.get("cooldown_until") is None:
        return False

    try:
        cooldown_until = pd.Timestamp(state["cooldown_until"])
        current_ts = pd.Timestamp(current_candle_time)
    except Exception as e:
        print(f"⚠️ V167.8.7 CIRCUIT BREAKER STATE UNPARSEABLE ({e}) — resetting")
        _reset_circuit_breaker_state("unparseable_timestamp")
        return False

    # Normalize both to tz-aware UTC
    try:
        ...
        print(f"⚠️ V167.8.7 CIRCUIT BREAKER TZ NORMALIZE FAIL ({e}) — resetting")
        _reset_circuit_breaker_state("tz_normalize_failed")
        return False

    # Sanity: cooldown_until must be a plausible future timestamp.
    # If it was written by the V167.8.4/5 bug, it will be year 1970.
    if cooldown_until.year < 2020:
        print(f"⚠️ V167.8.7 CIRCUIT BREAKER: legacy bad cooldown_until "
              f"({cooldown_until}) — resetting")
        _reset_circuit_breaker_state("legacy_1970_cooldown")
        return False
...
def final_signal(row, df=None):
    ...
    print()
    print("=" * 72)
    print("V167.8.7 ADAPTIVE ENTRY (EXIT REDESIGN)")
    print("=" * 72)
...
def print_state(row, signal, df=None):
    ...
    print()
    print("=" * 72)
    print("V167.8.7 PROFESSIONAL BOT - EXIT REDESIGN")
    print("=" * 72)
    print(f"  MODE            : {OPERATION_MODE}")
    print(f"  SYSTEM ID       : {SYSTEM_ID}")
...
def print_confluence_diagnostic(df, test_d):
    c = entry_confluence(df)
    print()
    print("V167.8.7 ENTRY CONFLUENCE (integrated as bounded bonus)")
...
def calculate_demo_tp_sl(signal, df, entry_price):
    ...
    print(f"📊 V167.8.7 DYNAMIC EXIT: SL_ATR={EXIT_SL_ATR_MULT_BASE:.2f} | "
...
def risk_guard(signal, df, entry_price, atr, sl, tp):
    ...
    if in_danger_window:
        ...
        if dangerous:
            ...
            print()
            print("=" * 72)
            print("V167.8.7 DANGER WINDOW CHECK")
            print("=" * 72)
    ...
    if reasons:
        print()
        print("=" * 72)
        print("V167.8.7 PRE-TRADE RISK GUARD")
        print("=" * 72)
...
def place_demo_market_order(signal, candle, df, row):
    ...
    try:
        ...
        if opened_pos is not None:
            ...
            print("⚠️ V167.8.7: could not read newly-opened position for entry_atr snapshot")
    except Exception as e:
        print(f"⚠️ V167.8.7: entry_atr snapshot save failed: {repr(e)}")
        ...
        try:
            ...
            if LAST_PRICE_TELEMETRY_ENABLED:
                ...
                if _LAST_PRICE_TELEMETRY["current_selected_path"] != path:
                    print(f"📡 V167.8.7 LAST_PRICE PATH CHANGED: now using '{path}' "
...
# ============================================================
# V167.8.7 — NEW: EFFECTIVE_OPENED_AT
# ============================================================
def _get_effective_opened_at(position, item=None):
    """
    V167.8.7:
      Returns a trustworthy tz-aware UTC timestamp for a position's
      open time, with explicit precedence and sanity checks:

        1. item["opened_at"]            — if parseable and year >= 2020
        2. position["createdTime"]      — exchange-provided, in ms
        3. pd.Timestamp.now(tz="UTC")   — last-resort fallback (flagged)

      Returns:
        (opened_at_ts, source_str)
        source_str ∈ {"STATE", "POSITION", "FALLBACK"}

      Why:  prior versions occasionally stored a local "now()" as opened_at
      when createdTime was missing, causing age_hours ≈ 0 forever, which
      silently disabled both TIME_EXIT and TP_COMPRESSION.  This function
      is the single, testable source of truth for "how old is this trade?".
    """
    # 1) Try item["opened_at"]
    if item:
        ...
        if raw is not None:
            try:
                ...
                if ts.year >= 2020:
                    ...
                    print(f"⚠️ V167.8.7 opened_at in STATE looks corrupt "
                          f"({ts}) — falling through to position.createdTime")
            except Exception as e:
                print(f"⚠️ V167.8.7 opened_at parse failed in STATE ({raw!r}: {e}) "
                      f"— falling through to position.createdTime")

    # 2) Try position["createdTime"]
    if position:
        try:
            ...
            print(f"⚠️ V167.8.7 position.createdTime parse failed: {e!r}")

    # 3) Fallback
    return pd.Timestamp.now(tz="UTC"), "FALLBACK"
...
def _reconstruct_entry_atr_if_needed(item, position, df):
    ...
    if snap is not None:
        ...
        if snap_atr > 0 and snap_rd > 0:
            ...
            if state_rd <= 0 or state_rd < snap_rd * (1.0 - RISK_DISTANCE_SANITY_TOLERANCE):
                print(f"🔧 V167.8.7 SNAPSHOT-BASED REBUILD: state_rd={state_rd:.2f} "
    ...
    if history_atr is not None and history_atr > 0:
        ...
        state_rd = float(item.get("risk_distance", 0) or 0)
        if state_rd <= 0 or state_rd < history_rd * (1.0 - RISK_DISTANCE_SANITY_TOLERANCE):
            print(f"🔧 V167.8.7 HISTORY-BASED REBUILD: state_rd={state_rd:.2f} "
    ...
    state_rd = float(item.get("risk_distance", 0) or 0)
    if state_rd <= 0:
        print("🚫 V167.8.7 ENTRY_ATR_UNKNOWN — SAFE HOLD")
...
def _ensure_exit_state_for_position(item, position, df):
    if item:
        ...
        # V167.8.7: reconcile opened_at against live position.
        ...
        if old_ts is None or old_ts != effective_opened_at:
            ...
            print(f"🔧 V167.8.7 OPENED_AT RECONCILED: "
    ...
    if created_ms is not None:
        ...
        if snap is not None:
            ...
            if snap_atr > 0 and snap_rd > 0:
                ...
                source = "SNAPSHOT"
                print(f"🔧 V167.8.7 FRESH STATE: using SNAPSHOT "
                      f"entry_atr={entry_atr:.2f} risk_distance={risk_distance:.2f}")
    ...
    if entry_atr is None or risk_distance is None:
        ...
        if hist_atr is not None and hist_atr > 0:
            ...
            print(f"🔧 V167.8.7 FRESH STATE: using HISTORY_REBUILD "
                  f"entry_atr={entry_atr:.2f} risk_distance={risk_distance:.2f}")

    if risk_distance is None or risk_distance <= 0:
        size = _position_size(position)
        hard_sl = None
        # V167.8.7: single source of truth for opened_at
        effective_opened_at, opened_at_source = _get_effective_opened_at(position, None)
        opened_at_str = effective_opened_at.isoformat()
        print("🚫 V167.8.7 FRESH STATE: ENTRY_ATR_UNKNOWN — SAFE HOLD")
...
def _record_close_for_circuit_breaker(item, reason, position, candle_time):
    """
    V167.8.7:
      - candle_time MUST be a real timestamp (pd.Timestamp / ISO string).
        Callers in manage_position_exits now pass pd.Timestamp.now(tz="UTC")
        instead of `mark`.
      - TRADE_CLOSED observation event is preserved (from V167.8.5),
        wrapped in its own try/except so it can never affect the circuit
        breaker update below.
    """
    try:
        entry_price_cb = float((item or {}).get("entry_price", 0) or 0)
        risk_distance_cb = float((item or {}).get("risk_distance", 0) or 0)
        side_cb = str((item or {}).get("side", "")).upper()
        exit_price_cb = None
        try:
            if isinstance(candle_time, pd.Timestamp):
                exit_price_cb = None
            else:
                exit_price_cb = float(candle_time)
        except Exception:
            exit_price_cb = None
        final_r = None
        if entry_price_cb > 0 and risk_distance_cb > 0 and exit_price_cb is not None:
            final_r = ((exit_price_cb - entry_price_cb) / risk_distance_cb) if side_cb == "LONG" \
                else ((entry_price_cb - exit_price_cb) / risk_distance_cb)
        was_invalidated = bool((item or {}).get("thesis_invalidated_at"))
        _exit_log_event({
            "event": "TRADE_CLOSED",
            "reason": reason,
            "side": side_cb,
            "entry_price": entry_price_cb if entry_price_cb > 0 else None,
            "exit_price": exit_price_cb,
        ...
        print(f"⚠️ V167.8.7 TRADE_CLOSED LOG ERROR (non-fatal, circuit breaker unaffected): {repr(e)}")

    try:
        if reason == "SL":
            update_circuit_breaker(True, candle_time)
            print("🧮 CIRCUIT BREAKER: loss recorded (SL)")
            return
        if reason == "BE":
            print("🧮 CIRCUIT BREAKER: BE scratch (streak unchanged)")
            return
        if reason in {"TRAIL", "TIMEEXIT"}:
            update_circuit_breaker(False, candle_time)
...
def manage_position_exits(df=None):
    ...
    try:
        ...
        for live_position in active:
                ...
                if verified:
                    ...
                    if pending_reason in {"TRAIL", "BE", "TIMEEXIT"}:
                        _record_close_for_circuit_breaker(
                            item, pending_reason, position, pd.Timestamp.now(tz="UTC")
                        )
                        state.pop(key, None)
                        _exit_save_state(state)
                        continue
                    ...
                    if position is None:
                        _record_close_for_circuit_breaker(
                            item, pending_reason, position or {}, pd.Timestamp.now(tz="UTC")
                        )
                        state.pop(key, None)
                        _exit_save_state(state)
                        continue
            ...
            # ============================================================
            # V167.8.7 THESIS OBSERVATION (LOG ONLY)
            # ============================================================
            ...
            if THESIS_OBSERVATION_ENABLED and not item.get("tp1_done", False) and risk_distance > 0:
                # V167.8.7: use effective opened_at (STATE → POSITION → FALLBACK)
                ...
                if opened_at_src_obs != "STATE":
                    print(f"🔧 V167.8.7 THESIS_AGE source={opened_at_src_obs} "
                ...
                try:
                    ...
                    print(f"⚠️ V167.8.7 THESIS OBSERVATION error (ignoring, log only): {repr(e)}")
                    thesis_valid_obs = item.get("thesis_valid", True)
                ...
                if not thesis_valid_obs and item.get("thesis_invalidated_at") is None:
                    ...
                    print(
                        f"🔎 V167.8.7 THESIS INVALIDATED (log only, no exit-behavior change): "
            ...
            # ============================================================
            # V167.8.7 DYNAMIC TP COMPRESSION — every iteration
            # ============================================================
            if (TP_COMPRESSION_ENABLED 
                ...
                age_hours = (pd.Timestamp.now(tz="UTC") - opened_at).total_seconds() / 3600
                if opened_at_src != "STATE":
                    print(f"🔧 V167.8.7 TP_COMPRESSION_AGE source={opened_at_src} "
                          f"opened_at={opened_at.isoformat()} age={age_hours:.2f}h")
                item["opened_at_source"] = opened_at_src
                if item.get("opened_at") != opened_at.isoformat():
                ...
                if age_hours >= TP_COMPRESSION_MIN_AGE_HOURS and atr_ratio < TP_COMPRESSION_ATR_RATIO:
                    if thesis_valid:
                        ...
                        _exit_save_state(state)
                        print(
                            f"📉 V167.8.7 TP COMPRESSED: age={age_hours:.1f}h "
                        ...
                        _exit_save_state(state)
                        print(
                            f"⚠️ V167.8.7 TP COMPRESSION SKIPPED: age={age_hours:.1f}h "
            ...
            if HARD_SL_WATCHDOG_ENABLED and hard_sl > 0 and sl_hit and size >= MIN_QTY:
                ...
                if verified:
                    ...
                    _record_close_for_circuit_breaker(
                        item, "SL", position, pd.Timestamp.now(tz="UTC")
                    )
                    state.pop(key, None)
                    _exit_save_state(state)
            ...
            if TIME_EXIT_ENABLED:
                ...
                if opened_at_src_te != "STATE":
                    print(f"🔧 V167.8.7 TIME_EXIT_AGE source={opened_at_src_te} "
                    ...
                    state[key] = item
                    _exit_save_state(state)
                if hours_open >= TIME_EXIT_HOURS and current_r >= TIME_EXIT_MIN_PROFIT_R:
                    ...
                    if verified:
                        _record_close_for_circuit_breaker(
                            item, "TIMEEXIT", position, pd.Timestamp.now(tz="UTC")
                        )
                        state.pop(key, None)
                    else:
            ...
            if TRAILING_ENABLED and item.get("tp2_done", False):
                ...
                if item.get("trailing_active", False):
                    ...
                    if hit and size >= MIN_QTY:
                        ...
                        if verified:
                            _record_close_for_circuit_breaker(
                                item, "TRAIL", position, pd.Timestamp.now(tz="UTC")
                            )
                            state.pop(key, None)
                        else:
            ...
            if item.get("be_armed", False):
                ...
                if be_hit and size >= MIN_QTY:
                    ...
                    if verified:
                        _record_close_for_circuit_breaker(
                            item, "BE", position, pd.Timestamp.now(tz="UTC")
                        )
                        state.pop(key, None)
                    else:
...
def main():
    try:
        ...
        print()
        print("=" * 72)
        print("V167.8.7 PROFESSIONAL BOT")
        ...
        print("V167.8.7 ADDITIONS : EFFECTIVE_OPENED_AT (single source of truth)")
        ...
        while True:
            try:
                ...
                print()
                print("=" * 72)
                print("V167.8.7 DEMO BOT STOPPED")
                print("=" * 72)
                break
            except Exception as e:
                print()
