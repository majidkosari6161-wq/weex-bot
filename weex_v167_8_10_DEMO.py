# V167.8.10 ONLINE + DEMO BUILD
# Default mode = WEEX DEMO live runner (PRIMARY).
# Set V167_OFFLINE_TEST=1 only for offline self-tests.
# ------------------------------------------------------------------
# CHANGES FROM V167.8.9:
#   [V167.8.10-1] FIXED: _modify_exchange_tpsl now requires & sends orderId.
#                         WEEX /capi/v3/modifyTpSlOrder returns -1141 without it.
#   [V167.8.10-2] FIXED: exchange_order_id extracted from order response and
#                         passed into post-fill TP/SL modify.
#   [V167.8.10-3] FIXED: get_open_demo_orders() tries both Demo symbol and
#                         contract symbol, with a 5-min negative cache to
#                         stop per-iteration -1142 spam.
#   [V167.8.10-4] FIXED: get_demo_last_price() has a per-endpoint 5-min
#                         negative cache for 4xx failures (stops symbolPrice spam).
#   [V167.8.10-5] All V167.8.9 fixes remain in place.
# ------------------------------------------------------------------
...
def _save_entry_atr_snapshot(symbol, side, created_ts_ms, entry_atr, entry_price, risk_distance):
    try:
        ...
        print(f"💾 V167.8.10 ENTRY_ATR SNAPSHOT SAVED: key={key} "
...
def _cleanup_entry_atr_snapshots(active_keys):
    try:
        ...
        os.replace(tmp, ENTRY_ATR_FILE)
        if to_remove:
            print(f"🧹 V167.8.10 ENTRY_ATR CLEANUP: removed {len(to_remove)} stale snapshot(s)")
    except Exception as e:
...
def _rebuild_entry_atr_from_history(df, created_ts):
    ...
    try:
        ...
        if pd.isna(atr_at_open):
            print(f"🚫 V167.8.10 HISTORY ATR: NaN at candle {entry_candle_ts}")
            return None
        atr_val = float(atr_at_open)
        if not np.isfinite(atr_val) or atr_val <= 0:
            print(f"🚫 V167.8.10 HISTORY ATR: invalid value {atr_val}")
            return None

        entry_close = float(x.loc[entry_candle_ts, "close"])
        if entry_close <= 0:
            print(f"🚫 V167.8.10 HISTORY ATR: entry_close={entry_close} invalid")
            return None
        atr_pct = atr_val / entry_close
        if atr_pct < ATR_SANITY_MIN_PCT or atr_pct > ATR_SANITY_MAX_PCT:
            print(f"🚫 V167.8.10 HISTORY ATR SANITY FAIL: "
        ...
        print(f"✅ V167.8.10 HISTORY ATR VERIFIED: candle={entry_candle_ts} "
...
def _risk_distance_is_sane(current_risk_distance, trusted_risk_distance,
    ...
    """V167.8.10: symmetric recovery sanity check (single helper)."""
    try:
...
def auth_headers(timestamp, signature):
    return {
        ...
        "User-Agent": "V167.8.10-Pro-Bot/1.0",
    }
...
def get_decayed_params():
    ...
    print(f"🔀 V167.8.10 DECAY: new params = {params}")
    return params
...
def final_signal(row, df=None):
    ...
    print()
    print("=" * 72)
    print("V167.8.10 ADAPTIVE ENTRY (EXIT REDESIGN)")
    print("=" * 72)
...
def print_state(row, signal, df=None):
    ...
    print()
    print("=" * 72)
    print("V167.8.10 PROFESSIONAL BOT - EXIT REDESIGN")
    print("=" * 72)
    print(f"  MODE            : {OPERATION_MODE}")
    print(f"  SYSTEM ID       : {SYSTEM_ID}")
...
def print_confluence_diagnostic(df, test_d):
    c = entry_confluence(df)
    print()
    print("V167.8.10 ENTRY CONFLUENCE (integrated as bounded bonus)")
...
def _get_effective_opened_at(position, item=None):
    """V167.8.10: only trusted timestamps. Never fabricate age with now()."""
    if item:
...
def calculate_demo_tp_sl(signal, df, entry_price):
    ...
    print(f"📊 V167.8.10 DYNAMIC EXIT: SL_ATR={EXIT_SL_ATR_MULT_BASE:.2f} | "
...
# ============================================================
# V167.8.10-1: MODIFY EXCHANGE TP/SL (requires orderId)
# ============================================================
def _modify_exchange_tpsl(symbol, position_side, new_tp, new_sl, order_id=None):
    """V167.8.10: modify existing exchange TP/SL after actual fill.

    WEEX API /capi/v3/modifyTpSlOrder REQUIRES 'orderId' (the id of the
    original MARKET order whose attached TP/SL we want to update).
    If orderId is missing, we skip the call and rely on the failsafe
    which was placed atomically with the original order.
    """
    if order_id is None:
        print("⚠️ V167.8.10 EXCHANGE TP/SL MODIFY SKIPPED: no orderId (failsafe remains)")
        return None
    body = {
        "symbol": symbol,
...
def risk_guard(signal, df, entry_price, atr, sl, tp):
    ...
    if in_danger_window:
        ...
        if dangerous:
            ...
            print()
            print("=" * 72)
            print("V167.8.10 DANGER WINDOW CHECK")
            print("=" * 72)
    ...
    if reasons:
        print()
        print("=" * 72)
        print("V167.8.10 PRE-TRADE RISK GUARD")
        print("=" * 72)
...
# ============================================================
# PLACE DEMO MARKET ORDER
# (V167.8.10-2: extract orderId; V167.8.10-3: pass it to modify)
# ============================================================
def place_demo_market_order(signal, candle, df, row):
    ...
    # ============================================================
    # V167.8.10-2: EXTRACT EXCHANGE ORDER ID
    # ============================================================
    ...
    if exchange_order_id:
        print(f"🔑 V167.8.10 EXCHANGE ORDER ID: {exchange_order_id}")
    else:
        print("⚠️ V167.8.10 EXCHANGE ORDER ID missing in response")

    # ============================================================
    # POST-FILL RECONCILE + EXCHANGE TP/SL MODIFY
    # ============================================================
    try:
        time.sleep(0.5)
        ...
        if opened_pos is not None:
            ...
            if actual_entry and actual_entry > 0:
                ...
                # ---- V167.8.10-3: modify exchange-side TP/SL with orderId ----
                try:
                    ...
                    if modify_result is not None:
                        print(
                            f"🔧 V167.8.10 EXCHANGE TP/SL MODIFIED: "
                    ...
                    print(
                        f"⚠️ V167.8.10 EXCHANGE TP/SL MODIFY FAILED "
            ...
            print("⚠️ V167.8.10: could not read newly-opened position for entry_atr snapshot")
    except Exception as e:
        print(f"⚠️ V167.8.10: entry_atr snapshot save failed: {repr(e)}")
...
# ============================================================
# V167.8.10-3: EXCHANGE PROTECTION RECONCILIATION
# (dual-symbol + 5-min negative cache)
# ============================================================
_OPEN_ORDERS_NEG_CACHE = {"ts": 0.0, "reason": None}
_OPEN_ORDERS_NEG_TTL = 300.0  # seconds (5 minutes)


def get_open_demo_orders():
    """V167.8.10: try Demo symbol first, then contract symbol.

    If both fail, enter a 5-minute negative cache to stop per-iteration spam.
    """
    now = time.time()
    if _OPEN_ORDERS_NEG_CACHE["reason"] and now - _OPEN_ORDERS_NEG_CACHE["ts"] < _OPEN_ORDERS_NEG_TTL:
        return []
    last_err = None
    ...
    print(f"⚠️ V167.8.10 openOrders unsupported on demo ({last_err}); cache 5min")
    return []
...
# ============================================================
# LAST PRICE (V167.8.10-5: per-endpoint 5-min negative cache)
# ============================================================
...
def get_demo_last_price(force_refresh=False):
    ...
    for path, params, key in WEEX_LAST_PRICE_PATHS:
        # V167.8.10-5: skip endpoint if it failed recently (5-min negative cache)
        ...
        try:
            ...
            if not r.ok:
                ...
                # V167.8.10-5: negative cache 5 min on 4xx
            ...
            if LAST_PRICE_TELEMETRY_ENABLED:
                ...
                if _LAST_PRICE_TELEMETRY["current_selected_path"] != path:
                    print(f"📡 V167.8.10 LAST_PRICE PATH CHANGED: now using '{path}' "
...
def get_circuit_breaker_state():
    """V167.8.10: corrupt CB state is quarantined, never silently reset."""
...
# ============================================================
# RISK_DISTANCE SANITY CHECK + REBUILD (symmetric helper)
# ============================================================
def _reconstruct_entry_atr_if_needed(item, position, df):
    ...
    if snap is not None:
        ...
        if snap_atr > 0 and snap_rd > 0:
            ...
            if state_rd <= 0 or not _risk_distance_is_sane(state_rd, snap_rd):
                print(f"🔧 V167.8.10 SNAPSHOT-BASED REBUILD: state_rd={state_rd:.2f} "
    ...
    if history_atr is not None and history_atr > 0:
        ...
        if state_rd <= 0 or not _risk_distance_is_sane(state_rd, history_rd):
            print(f"🔧 V167.8.10 HISTORY-BASED REBUILD: state_rd={state_rd:.2f} "
    ...
    state_rd = float(item.get("risk_distance", 0) or 0)
    if state_rd <= 0:
        print("🚫 V167.8.10 ENTRY_ATR_UNKNOWN — SAFE HOLD")
...
def _ensure_exit_state_for_position(item, position, df):
    if item:
        ...
        if old_ts is None or old_ts != effective_opened_at:
            ...
            print(f"🔧 V167.8.10 OPENED_AT RECONCILED: "
    ...
    if created_ms is not None:
        ...
        if snap is not None:
            ...
            if snap_atr > 0 and snap_rd > 0:
                ...
                source = "SNAPSHOT"
                print(f"🔧 V167.8.10 FRESH STATE: using SNAPSHOT "
                      f"entry_atr={entry_atr:.2f} risk_distance={risk_distance:.2f}")
    ...
    if entry_atr is None or risk_distance is None:
        ...
        if hist_atr is not None and hist_atr > 0:
            ...
            print(f"🔧 V167.8.10 FRESH STATE: using HISTORY_REBUILD "
                  f"entry_atr={entry_atr:.2f} risk_distance={risk_distance:.2f}")
    ...
    if risk_distance is None or risk_distance <= 0:
        ...
        print("🚫 V167.8.10 FRESH STATE: ENTRY_ATR_UNKNOWN — SAFE HOLD")
...
def _record_close_for_circuit_breaker(item, reason, position, closed_at=None, exit_price=None):
    """V167.8.10: closed_at and exit_price are separate, typed values."""
...
# ============================================================
# MANAGE POSITION EXITS (V167.8.10: trailing tied to tp2_r_effective)
# ============================================================
        ...
        for live_position in active:
            ...
            if THESIS_OBSERVATION_ENABLED and not item.get("tp1_done", False) and risk_distance > 0:
                ...
                if opened_at_src_obs != "STATE":
                    print(f"🔧 V167.8.10 THESIS_AGE source={opened_at_src_obs} "
                ...
                try:
                    ...
                    print(f"⚠️ V167.8.10 THESIS OBSERVATION error (ignoring, log only): {repr(e)}")
                    thesis_valid_obs = item.get("thesis_valid", True)
                ...
                if not thesis_valid_obs and item.get("thesis_invalidated_at") is None:
                    ...
                    print(
                        f"🔎 V167.8.10 THESIS INVALIDATED (log only, no exit-behavior change): "
            ...
            if (TP_COMPRESSION_ENABLED 
                ...
                if opened_at_src != "STATE" and opened_at is not None:
                    print(f"🔧 V167.8.10 TP_COMPRESSION_AGE source={opened_at_src} "
                ...
                if age_hours is not None and age_hours >= TP_COMPRESSION_MIN_AGE_HOURS and atr_ratio < TP_COMPRESSION_ATR_RATIO:
                    if thesis_valid:
                        ...
                        _exit_save_state(state)
                        print(
                            f"📉 V167.8.10 TP COMPRESSED: age={age_hours:.1f}h "
                        ...
                        _exit_save_state(state)
                        print(
                            f"⚠️ V167.8.10 TP COMPRESSION SKIPPED: age={age_hours:.1f}h "
            ...
            if TIME_EXIT_ENABLED:
                ...
                if opened_at_src_te != "STATE" and opened_at_te is not None:
                    print(f"🔧 V167.8.10 TIME_EXIT_AGE source={opened_at_src_te} "
            ...
            # ============================================================
            # V167.8.10: TRAILING tied to effective TP2
            # ============================================================
...
def main():
    try:
        ...
        print()
        print("=" * 72)
        print("V167.8.10 PROFESSIONAL BOT  [ONLINE DEMO MODE]")
        print("  + FIXED: modifyTpSlOrder now sends orderId (V167.8.10-1)")
        print("  + FIXED: exchange_order_id extracted from order response (V167.8.10-2)")
        print("  + FIXED: openOrders dual-symbol + 5min negative cache (V167.8.10-3)")
        print("  + FIXED: LAST_PRICE per-endpoint negative cache (V167.8.10-4)")
        ...
        print("V167.8.10 ADDITIONS: modifyTpSlOrder orderId + openOrders dual-symbol + neg caches")
        ...
        print("EXCHANGE TP/SL MOD : ENABLED (requires orderId; falls back to failsafe)")
        print("OPEN ORDERS QUERY  : dual-symbol + 5min neg cache")
        ...
        while True:
            try:
                ...
                print()
                print("=" * 72)
                print("V167.8.10 DEMO BOT STOPPED")
                print("=" * 72)
                break
            except Exception as e:
                print()
...
def _offline_self_tests():
    ...
    print("\n" + "=" * 72)
    print("V167.8.10 OFFLINE SELF TEST")
    print("=" * 72)
    passed = 0
...
if __name__ == "__main__":
    ...
    if os.getenv("V167_OFFLINE_TEST", "0").strip().lower() in {"1", "true", "yes", "on"}:
        print("=" * 72)
        print("V167.8.10 OFFLINE SELF-TEST MODE")
