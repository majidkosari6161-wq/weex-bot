# V167.8.8 OFFLINE TEST BUILD
# Exchange connectivity/order execution is disabled.
# This build is intended for local software/recovery testing only.
...
#   [V167.8.8-1] NEW: _get_effective_opened_at(position, item)
...
#   [V167.8.8-2] RECONCILIATION in _ensure_exit_state_for_position:
...
#   [V167.8.8-3] THESIS OBSERVATION now uses _get_effective_opened_at.
#   [V167.8.8-4] TP COMPRESSION now uses _get_effective_opened_at.
#   [V167.8.8-5] TIME EXIT now uses _get_effective_opened_at.
#
...
def _save_entry_atr_snapshot(symbol, side, created_ts_ms, entry_atr, entry_price, risk_distance):
    try:
        ...
        print(f"💾 V167.8.8 ENTRY_ATR SNAPSHOT SAVED: key={key} "
...
def _cleanup_entry_atr_snapshots(active_keys):
    try:
        ...
        os.replace(tmp, ENTRY_ATR_FILE)
        if to_remove:
            print(f"🧹 V167.8.8 ENTRY_ATR CLEANUP: removed {len(to_remove)} stale snapshot(s)")
    except Exception as e:
...
def _rebuild_entry_atr_from_history(df, created_ts):
    ...
    try:
        ...
        if pd.isna(atr_at_open):
            print(f"🚫 V167.8.8 HISTORY ATR: NaN at candle {entry_candle_ts}")
            return None
        atr_val = float(atr_at_open)
        if not np.isfinite(atr_val) or atr_val <= 0:
            print(f"🚫 V167.8.8 HISTORY ATR: invalid value {atr_val}")
            return None

        entry_close = float(x.loc[entry_candle_ts, "close"])
        if entry_close <= 0:
            print(f"🚫 V167.8.8 HISTORY ATR: entry_close={entry_close} invalid")
            return None
        atr_pct = atr_val / entry_close
        if atr_pct < ATR_SANITY_MIN_PCT or atr_pct > ATR_SANITY_MAX_PCT:
            print(f"🚫 V167.8.8 HISTORY ATR SANITY FAIL: "
        ...
        print(f"✅ V167.8.8 HISTORY ATR VERIFIED: candle={entry_candle_ts} "
...
def auth_headers(timestamp, signature):
    return {
        ...
        "User-Agent": "V167.8.8-Pro-Bot/1.0",
    }
...
def get_decayed_params():
    ...
    print(f"🔀 V167.8.8 DECAY: new params = {params}")
    return params
...
def _risk_distance_is_sane(current_risk_distance, trusted_risk_distance,
    ...
    """V167.8.8: symmetric recovery sanity check."""
    try:
...
# ============================================================
# CIRCUIT BREAKER (V167.8.8: hardened)
# ============================================================
def _atomic_json_write(path, payload):
    """V167.8.8: atomic JSON write used by safety-critical state."""
    path = Path(path)
...
def get_circuit_breaker_state():
    """V167.8.8: corrupt CB state is quarantined, never silently reset."""
...
def final_signal(row, df=None):
    ...
    print()
    print("=" * 72)
    print("V167.8.8 ADAPTIVE ENTRY (EXIT REDESIGN)")
    print("=" * 72)
...
def print_state(row, signal, df=None):
    ...
    print()
    print("=" * 72)
    print("V167.8.8 PROFESSIONAL BOT - EXIT REDESIGN")
    print("=" * 72)
    print(f"  MODE            : {OPERATION_MODE}")
    print(f"  SYSTEM ID       : {SYSTEM_ID}")
...
def print_confluence_diagnostic(df, test_d):
    c = entry_confluence(df)
    print()
    print("V167.8.8 ENTRY CONFLUENCE (integrated as bounded bonus)")
...
def calculate_demo_tp_sl(signal, df, entry_price):
    ...
    print(f"📊 V167.8.8 DYNAMIC EXIT: SL_ATR={EXIT_SL_ATR_MULT_BASE:.2f} | "
...
def risk_guard(signal, df, entry_price, atr, sl, tp):
    ...
    if in_danger_window:
        ...
        if dangerous:
            ...
            print()
            print("=" * 72)
            print("V167.8.8 DANGER WINDOW CHECK")
            print("=" * 72)
    ...
    if reasons:
        print()
        print("=" * 72)
        print("V167.8.8 PRE-TRADE RISK GUARD")
        print("=" * 72)
...
def place_demo_market_order(signal, candle, df, row):
    ...
    try:
        ...
        if opened_pos is not None:
            ...
            print("⚠️ V167.8.8: could not read newly-opened position for entry_atr snapshot")
    except Exception as e:
        print(f"⚠️ V167.8.8: entry_atr snapshot save failed: {repr(e)}")
...
def get_open_demo_orders():
    """V167.8.8: current regular orders. TP/SL conditional orders are also inspected when exposed by API."""
    try:
...
def reconcile_exchange_protection(position, item):
    """V167.8.8: verify that expected exchange protection is present when API exposes it.
        ...
        try:
            ...
            if LAST_PRICE_TELEMETRY_ENABLED:
                ...
                if _LAST_PRICE_TELEMETRY["current_selected_path"] != path:
                    print(f"📡 V167.8.8 LAST_PRICE PATH CHANGED: now using '{path}' "
...
# ============================================================
# V167.8.8 — NEW: EFFECTIVE_OPENED_AT
# ============================================================
def _get_effective_opened_at(position, item=None):
    """V167.8.8: only trusted timestamps. Never fabricate age with now()."""
    if item:
...
def _reconstruct_entry_atr_if_needed(item, position, df):
    ...
    if snap is not None:
        ...
        if snap_atr > 0 and snap_rd > 0:
            ...
            if state_rd <= 0 or state_rd < snap_rd * RISK_DISTANCE_SANITY_LOW or state_rd > snap_rd * RISK_DISTANCE_SANITY_HIGH:
                print(f"🔧 V167.8.8 SNAPSHOT-BASED REBUILD: state_rd={state_rd:.2f} "
    ...
    if history_atr is not None and history_atr > 0:
        ...
        if state_rd <= 0 or state_rd < history_rd * RISK_DISTANCE_SANITY_LOW or state_rd > history_rd * RISK_DISTANCE_SANITY_HIGH:
            print(f"🔧 V167.8.8 HISTORY-BASED REBUILD: state_rd={state_rd:.2f} "
    ...
    state_rd = float(item.get("risk_distance", 0) or 0)
    if state_rd <= 0:
        print("🚫 V167.8.8 ENTRY_ATR_UNKNOWN — SAFE HOLD")
...
def _ensure_exit_state_for_position(item, position, df):
    if item:
        ...
        # V167.8.8: reconcile opened_at against live position.
        ...
        if old_ts is None or old_ts != effective_opened_at:
            ...
            print(f"🔧 V167.8.8 OPENED_AT RECONCILED: "
    ...
    if created_ms is not None:
        ...
        if snap is not None:
            ...
            if snap_atr > 0 and snap_rd > 0:
                ...
                source = "SNAPSHOT"
                print(f"🔧 V167.8.8 FRESH STATE: using SNAPSHOT "
                      f"entry_atr={entry_atr:.2f} risk_distance={risk_distance:.2f}")
    ...
    if entry_atr is None or risk_distance is None:
        ...
        if hist_atr is not None and hist_atr > 0:
            ...
            print(f"🔧 V167.8.8 FRESH STATE: using HISTORY_REBUILD "
                  f"entry_atr={entry_atr:.2f} risk_distance={risk_distance:.2f}")

    if risk_distance is None or risk_distance <= 0:
        size = _position_size(position)
        hard_sl = None
        # V167.8.8: single source of truth for opened_at
        ...
        print("🚫 V167.8.8 FRESH STATE: ENTRY_ATR_UNKNOWN — SAFE HOLD")
...
def _record_close_for_circuit_breaker(item, reason, position, closed_at=None, exit_price=None):
    """V167.8.8: closed_at and exit_price are separate, typed values."""
...
def manage_position_exits(df=None):
    ...
    try:
        ...
        for live_position in active:
            ...
            # ============================================================
            # V167.8.8 THESIS OBSERVATION (LOG ONLY)
            # ============================================================
            ...
            if THESIS_OBSERVATION_ENABLED and not item.get("tp1_done", False) and risk_distance > 0:
                # V167.8.8: use effective opened_at (STATE → POSITION → FALLBACK)
                ...
                if opened_at_src_obs != "STATE":
                    print(f"🔧 V167.8.8 THESIS_AGE source={opened_at_src_obs} "
                ...
                try:
                    ...
                    print(f"⚠️ V167.8.8 THESIS OBSERVATION error (ignoring, log only): {repr(e)}")
                    thesis_valid_obs = item.get("thesis_valid", True)
                ...
                if not thesis_valid_obs and item.get("thesis_invalidated_at") is None:
                    ...
                    print(
                        f"🔎 V167.8.8 THESIS INVALIDATED (log only, no exit-behavior change): "
            ...
            # ============================================================
            # V167.8.8 DYNAMIC TP COMPRESSION — every iteration
            # ============================================================
            if (TP_COMPRESSION_ENABLED 
                ...
                if opened_at_src != "STATE" and opened_at is not None:
                    print(f"🔧 V167.8.8 TP_COMPRESSION_AGE source={opened_at_src} "
                ...
                if age_hours is not None and age_hours >= TP_COMPRESSION_MIN_AGE_HOURS and atr_ratio < TP_COMPRESSION_ATR_RATIO:
                    if thesis_valid:
                        ...
                        _exit_save_state(state)
                        print(
                            f"📉 V167.8.8 TP COMPRESSED: age={age_hours:.1f}h "
                        ...
                        _exit_save_state(state)
                        print(
                            f"⚠️ V167.8.8 TP COMPRESSION SKIPPED: age={age_hours:.1f}h "
            ...
            if TIME_EXIT_ENABLED:
                ...
                if opened_at_src_te != "STATE" and opened_at_te is not None:
                    print(f"🔧 V167.8.8 TIME_EXIT_AGE source={opened_at_src_te} "
...
def main():
    try:
        ...
        print()
        print("=" * 72)
        print("V167.8.8 PROFESSIONAL BOT")
        ...
        while True:
            try:
                ...
                print()
                print("=" * 72)
                print("V167.8.8 DEMO BOT STOPPED")
                print("=" * 72)
                break
            except Exception as e:
                print()
...
def _offline_self_tests():
    ...
    print("\n" + "=" * 72)
    print("V167.8.8 OFFLINE SELF TEST")
    print("=" * 72)
    passed = 0
...
if __name__ == "__main__":
    print("=" * 72)
    print("V167.8.8 OFFLINE TEST BUILD")
