...
# V167.8.12: independent notional cap, checked in addition to margin cap.
...
# V167.8.12: confirmed INVALID thesis may trigger an early exit before TP1.
...
# V167.8.11 HARDENING + V167.8.12 EXECUTION/EXIT SAFETY
...
def auth_headers(timestamp, signature):
    return {
        ...
        "User-Agent": "V167.8.11-Pro-Bot/1.0",
    }
...
def print_state(row, signal, df=None):
    ...
    print()
    print("=" * 72)
    print("V167.8.12 PROFESSIONAL BOT - EXIT REDESIGN")
    print("=" * 72)
    print(f"  MODE            : {OPERATION_MODE}")
    print(f"  SYSTEM ID       : {SYSTEM_ID}")
...
def calculate_position_size(entry_price, stop_loss_price, account_balance, risk_percent=None):
    """V167.8.12: qty = min(risk_based, margin_cap, notional_cap), with post-rounding re-check."""
    ...
    if qty > max_qty_after_round + 1e-12:
        ...
        print(
            f"🛡️ V167.8.12 POST-ROUNDING CAP RECHECK: "
    ...
    if margin_breach or notional_breach:
        ...
        corrected = float(round(corrected, 4))
        print(
            f"🛡️ V167.8.12 HARD POST-ROUNDING RECHECK: "
    ...
    if qty < MIN_QTY:
        raise RuntimeError(
            ...
            f"after applying V167.8.12 caps."
        )

    effective_risk_pct = (qty * risk_per_unit) / account_balance * 100
    print(
        f"✅ V167.8.12 FINAL QTY: {qty:.4f} | "
...
def place_demo_market_order(signal, candle, df, row):
    ...
    # V167.8.12: Minimum Risk Distance Guard.
    ...
    if risk_distance_preflight < min_risk_distance:
        print()
        print("=" * 72)
        print("V167.8.12 MIN RISK DISTANCE GUARD")
        print("=" * 72)
    ...
    # V167.8.12: final pre-flight recheck against BOTH independent account caps.
    ...
    if qty > hard_cap_qty + 1e-12:
        ...
        print(
            f"🛡️ V167.8.12 PRE-FLIGHT CAP RECHECK: qty={qty:.4f} > "
            f"hard_cap={hard_cap_qty:.4f} | shrinking → {safe_qty:.4f}"
        )
        if safe_qty < MIN_QTY:
            raise RuntimeError(
                f"Available {available_balance:.4f} SUSDT too small for "
                f"MIN_QTY={MIN_QTY} after V167.8.12 margin/notional caps."
            )
        qty = safe_qty
        actual_margin = (qty * entry_price) / MAX_LEVERAGE
        actual_notional = qty * entry_price
    ...
    if actual_margin > max_allowed_margin + 1e-9 or actual_notional > max_allowed_notional + 1e-9:
        raise RuntimeError(
            f"V167.8.12 sizing guard failure: margin={actual_margin:.4f}/{max_allowed_margin:.4f}, "
        ...
        if opened_pos is not None:
            ...
            if actual_entry and actual_entry > 0:
                ...
                _exit_log_event({
                    ...
                    "exchange_protection_verification": "NOT_EXPOSED_BY_DEMO_OPEN_ORDERS_ENDPOINT",
                })
                ...
                # V167.8.12: three-tier slippage guard.
                try:
                    ...
                    print(
                        f"{slip_icon} V167.8.12 SLIPPAGE GUARD [{slip_tier}]: "
                    ...
                    if slip_tier == "EXTREME" and slippage_risk_ratio >= SLIPPAGE_EXTREME_RISK_RATIO:
                        print(
                            f"🛑 V167.8.12 EXTREME SLIPPAGE — EMERGENCY EXIT "
                        ...
                        print("🟡 V167.8.12 WARNING SLIPPAGE — trade continues; actual fill remains source of truth")
                except Exception as slip_err:
                    print(f"⚠️ V167.8.12 SLIPPAGE GUARD ERROR (ignoring): {slip_err!r}")

                print(
                    f"🛡️ V167.8.11 POST-FILL PROTECTION: expected entry-order "
                    f"TP={actual_ftp:.1f} SL={actual_fsl:.1f}; "
                    f"invalid post-fill modify request suppressed."
                )
        else:
...
def reconcile_exchange_protection(position, item):
    """V167.8.11: verify that expected exchange protection is present when API exposes it.
    We never invent an order id. If the exchange does not expose a conditional order in
    openOrders, the bot records UNKNOWN rather than deleting protection or placing a duplicate.
    """
        ...
        for live_position in active:
            ...
            if THESIS_OBSERVATION_ENABLED and not item.get("tp1_done", False) and risk_distance > 0:
                ...
                if not thesis_valid_obs and item.get("thesis_invalidated_at") is None:
                    ...
                    print(
                        f"🔎 V167.8.12 THESIS INVALIDATED (observation + early-exit guard): "
                ...
                # ============================================================
                # V167.8.12 THESIS EARLY EXIT — confirmed INVALID only
                # ============================================================
                if THESIS_EARLY_EXIT_ENABLED and not item.get("tp1_done", False):
                    ...
                    if current_status == "INVALID":
                        ...
                        if (age_minutes is not None
                            ...
                            print(
                                f"🛑 V167.8.12 THESIS EARLY EXIT CONFIRMED: "
                            ...
                            try:
                                ...
                                print(f"⚠️ V167.8.12 THESIS EARLY EXIT SEND ERROR: {thesis_exit_err!r}")
...
def main():
    try:
        ...
        print()
        print("=" * 72)
        print("V167.8.12 PROFESSIONAL BOT")
        ...
        print("V167.8.12 ADDITIONS : EXECUTION/EXIT SAFETY (4 GUARDS)")
        ...
        while True:
            try:
                ...
                print()
                print("=" * 72)
                print("V167.8.12 DEMO BOT STOPPED")
                print("=" * 72)
                break
            except Exception as e:
                print()
...
def _offline_self_tests():
    ...
    print("\n" + "=" * 72)
    print("V167.8.12 OFFLINE SELF TEST")
    print("=" * 72)
    passed = 0
...
if __name__ == "__main__":
    ...
    if os.getenv("V167_OFFLINE_TEST", "0").strip().lower() in {"1", "true", "yes", "on"}:
        print("=" * 72)
        print("V167.8.12 OFFLINE SELF-TEST MODE")
