# V167.8.11 DEMO + OFFLINE SELF-TEST BUILD
# Default execution mode is the actual WEEX DEMO runner.
# Offline self-tests are opt-in via V167_OFFLINE_TEST=1.
...
# ============================================================
# V167.8.13.1 PROFESSIONAL BOT
#   - EFFECTIVE_OPENED_AT (STATE → POSITION → FALLBACK)
#   - CIRCUIT BREAKER TIMESTAMP FIX (root cause + defense in depth)
#   - OBSERVATION / TELEMETRY (thesis + last_price + trade_closed)
#   - HISTORICAL ENTRY-ATR VERIFICATION
#   - SAFE HOLD (no current-ATR fallback)
#   - PERSISTENT ENTRY_ATR SNAPSHOT
#   - RISK_DISTANCE SANITY CHECK + REBUILD
#   - PER-ITERATION TP COMPRESSION RESTORE
#   - EXIT STATE RESILIENCE
#   - DYNAMIC TP COMPRESSION + THESIS VALIDITY GUARD
#   - DUAL-PRICE HARD-SL + LAST_PRICE FAILSAFE + ADAPTIVE POLLING
#   - CONDITION-AWARE SESSION FILTER
#   - FEE-AWARE BREAKEVEN + MARGIN BUFFER
# ============================================================
#
# CHANGES FROM V167.8.6:
#   [V167.8.11-1] NEW: _get_effective_opened_at(position, item)
#       Single source of truth for trade age. Precedence:
#         1. item["opened_at"]         if parseable and year >= 2020
#         2. position["createdTime"]   exchange-provided, ms epoch
#         3. pd.Timestamp.now(tz="UTC") last-resort (flagged as FALLBACK)
#       Heals stale/corrupt/zero opened_at without losing valid values.
#       Prevents silent disabling of TIME_EXIT and TP_COMPRESSION that
#       occurred when opened_at ≈ now() forever.
#   [V167.8.11-2] RECONCILIATION in _ensure_exit_state_for_position:
#       Both the "existing item" branch and the "fresh state" branch now
#       store opened_at from _get_effective_opened_at, plus an
#       opened_at_source field ("STATE" | "POSITION" | "FALLBACK").
#   [V167.8.11-3] THESIS OBSERVATION now uses _get_effective_opened_at.
#   [V167.8.11-4] TP COMPRESSION now uses _get_effective_opened_at.
#   [V167.8.11-5] TIME EXIT now uses _get_effective_opened_at.
#
# ENTRY : Unchanged from V167.8.6.
# EXIT  : Behavior unchanged except for corrected trade-age calculation.
# ============================================================
...
# V167.8.13: independent notional cap, checked in addition to margin cap.
...
# V167.8.13: confirmed INVALID thesis may trigger an early exit before TP1.
THESIS_EARLY_EXIT_ENABLED = True
THESIS_EARLY_EXIT_WINDOW_MINUTES = 5
THESIS_INVALID_CONFIRMATIONS_REQUIRED = 2
THESIS_INVALID_CONFIRM_MIN_INTERVAL_SECONDS = 2.0
...
# V167.8.11 HARDENING + V167.8.13 EXECUTION/EXIT SAFETY
...
# V167.8.13: Minimum Risk Distance Guard (price pct + entry ATR).
MIN_RISK_DISTANCE_PCT = 0.005       # 0.50% of entry price
MIN_RISK_DISTANCE_ATR_MULT = 1.50   # 1.50 x entry ATR

# V167.8.13: Slippage Guard (three-tier).
SLIPPAGE_NORMAL_MAX_PCT = 0.0010    # 0.10%
SLIPPAGE_WARNING_MAX_PCT = 0.0020   # 0.20%
SLIPPAGE_EXTREME_RISK_RATIO = 0.30  # 30% of risk_distance
...
def _save_entry_atr_snapshot(symbol, side, created_ts_ms, entry_atr, entry_price, risk_distance):
    try:
        ...
        print(f"💾 V167.8.11 ENTRY_ATR SNAPSHOT SAVED: key={key} "
...
def _cleanup_entry_atr_snapshots(active_keys):
    try:
        ...
        os.replace(tmp, ENTRY_ATR_FILE)
        if to_remove:
            print(f"🧹 V167.8.11 ENTRY_ATR CLEANUP: removed {len(to_remove)} stale snapshot(s)")
    except Exception as e:
...
def _rebuild_entry_atr_from_history(df, created_ts):
    ...
    try:
        ...
        if pd.isna(atr_at_open):
            print(f"🚫 V167.8.11 HISTORY ATR: NaN at candle {entry_candle_ts}")
            return None
        atr_val = float(atr_at_open)
        if not np.isfinite(atr_val) or atr_val <= 0:
            print(f"🚫 V167.8.11 HISTORY ATR: invalid value {atr_val}")
            return None

        entry_close = float(x.loc[entry_candle_ts, "close"])
        if entry_close <= 0:
            print(f"🚫 V167.8.11 HISTORY ATR: entry_close={entry_close} invalid")
            return None
        atr_pct = atr_val / entry_close
        if atr_pct < ATR_SANITY_MIN_PCT or atr_pct > ATR_SANITY_MAX_PCT:
            print(f"🚫 V167.8.11 HISTORY ATR SANITY FAIL: "
        ...
        print(f"✅ V167.8.11 HISTORY ATR VERIFIED: candle={entry_candle_ts} "
...
def auth_headers(timestamp, signature):
    return {
        ...
        "User-Agent": "V167.8.13-Pro-Bot/1.0",
    }
...
def get_decayed_params():
    ...
    print(f"🔀 V167.8.11 DECAY: new params = {params}")
    return params
...
def _risk_distance_is_sane(current_risk_distance, trusted_risk_distance,
    ...
    """V167.8.11: symmetric recovery sanity check."""
    try:
...
# ============================================================
# CIRCUIT BREAKER (V167.8.11: hardened)
# ============================================================
def _atomic_json_write(path, payload):
    """V167.8.11: atomic JSON write used by safety-critical state."""
    path = Path(path)
...
def get_circuit_breaker_state():
    """V167.8.11: corrupt CB state is quarantined, never silently reset."""
...
def update_circuit_breaker(is_loss, current_candle_time):
    ...
    if not state.get("state_valid", True):
        # Never silently clear a corrupted safety state. Require an explicit
        # clean initialization before trading can resume.
...
def final_signal(row, df=None):
    ...
    print()
    print("=" * 72)
    print("V167.8.11 ADAPTIVE ENTRY (EXIT REDESIGN)")
    print("=" * 72)
...
def print_state(row, signal, df=None):
    ...
    print()
    print("=" * 72)
    print("V167.8.13 PROFESSIONAL BOT - EXIT REDESIGN")
    print("=" * 72)
    print(f"  MODE            : {OPERATION_MODE}")
    print(f"  SYSTEM ID       : {SYSTEM_ID}")
...
def print_confluence_diagnostic(df, test_d):
    c = entry_confluence(df)
    print()
    print("V167.8.11 ENTRY CONFLUENCE (integrated as bounded bonus)")
    ...
    try:
        ...
        for order in sorted(relevant, key=lambda x: _order_time_utc(x) or pd.Timestamp(1970, unit="s", tz="UTC")):
            ...
            if "-TP1-" in cid:
                ...
                if px:
                    item["tp1_price"] = px
                    _record_exit_fill(item, "TP1", qty, px, order_id=oid, filled_at=_order_time_utc(order))
            elif "-TP2-" in cid:
                item["tp2_done"] = True
                if px:
                    item["tp2_price"] = px
                    _record_exit_fill(item, "TP2", qty, px, order_id=oid, filled_at=_order_time_utc(order))
        return item
    except Exception as e:
...
def calculate_position_size(entry_price, stop_loss_price, account_balance, risk_percent=None):
    """V167.8.13: qty = min(risk_based, margin_cap, notional_cap), with post-rounding re-check."""
    ...
    # Cap A: risk-based sizing.
    risk_based_qty = risk_amount / risk_per_unit

    # Cap B: margin utilization.
    max_margin = account_balance * MAX_MARGIN_UTILIZATION
    margin_cap_qty = (max_margin * MAX_LEVERAGE) / entry_price

    # Cap C: notional utilization.
    max_notional = account_balance * MAX_NOTIONAL_PCT * MAX_LEVERAGE
    notional_cap_qty = max_notional / entry_price

    raw_qty = min(risk_based_qty, margin_cap_qty, notional_cap_qty)

    print(
        f"📐 V167.8.13 POSITION SIZING: "
        f"risk_based={risk_based_qty:.4f} | "
        f"margin_cap={margin_cap_qty:.4f} | "
        f"notional_cap={notional_cap_qty:.4f} | "
        f"→ raw={raw_qty:.4f}"
    )

    qty = np.floor(raw_qty / QTY_STEP) * QTY_STEP
    qty = float(round(qty, 4))

    # Re-check after rounding. Use the minimum of all three caps again so the
    # order can never exceed a cap because of step rounding.
    max_qty_after_round = min(
        margin_cap_qty,
        notional_cap_qty,
        risk_based_qty,
    )
    if qty > max_qty_after_round + 1e-12:
        corrected = np.floor(max_qty_after_round / QTY_STEP) * QTY_STEP
        corrected = float(round(corrected, 4))
        print(
            f"🛡️ V167.8.13 POST-ROUNDING CAP RECHECK: "
            f"qty={qty:.4f} > cap={max_qty_after_round:.4f} → {corrected:.4f}"
        )
        qty = corrected

    actual_margin = (qty * entry_price) / MAX_LEVERAGE
    actual_notional = qty * entry_price
    margin_breach = actual_margin > max_margin + 1e-9
    notional_breach = actual_notional > max_notional + 1e-9
    if margin_breach or notional_breach:
        corrected_cap_qty = min(
            margin_cap_qty,
            notional_cap_qty,
        )
        corrected = np.floor(corrected_cap_qty / QTY_STEP) * QTY_STEP
        corrected = float(round(corrected, 4))
        print(
            f"🛡️ V167.8.13 HARD POST-ROUNDING RECHECK: "
            f"margin={actual_margin:.4f}/{max_margin:.4f}, "
            f"notional={actual_notional:.4f}/{max_notional:.4f} → qty={corrected:.4f}"
        )
        qty = corrected
        actual_margin = (qty * entry_price) / MAX_LEVERAGE
        actual_notional = qty * entry_price
    ...
    if qty < MIN_QTY:
        raise RuntimeError(
            f"Calculated quantity {qty} is below MIN_QTY {MIN_QTY} "
            f"after applying V167.8.13 caps."
        )

    effective_risk_pct = (qty * risk_per_unit) / account_balance * 100
    print(
        f"✅ V167.8.13 FINAL QTY: {qty:.4f} | "
        f"margin={actual_margin:.2f} ({actual_margin/account_balance*100:.1f}%) | "
        f"notional={actual_notional:.2f} | "
        f"effective_risk={effective_risk_pct:.2f}% (target {risk_percent*100:.2f}%)"
    )
    return qty
...
def calculate_demo_tp_sl(signal, df, entry_price):
    ...
    print(f"📊 V167.8.11 DYNAMIC EXIT: SL_ATR={EXIT_SL_ATR_MULT_BASE:.2f} | "
...
def risk_guard(signal, df, entry_price, atr, sl, tp):
    ...
    if in_danger_window:
        ...
        if dangerous:
            ...
            print()
            print("=" * 72)
            print("V167.8.11 DANGER WINDOW CHECK")
            print("=" * 72)
    ...
    if reasons:
        print()
        print("=" * 72)
        print("V167.8.11 PRE-TRADE RISK GUARD")
        print("=" * 72)
...
def _modify_exchange_tpsl(symbol, position_side, new_tp, new_sl, order_id=None):
    """V167.8.11: disabled for DEMO because the V3 endpoint modifies one conditional order.

    The market orderId returned by /capi/v3/sim/order is not the conditional TP/SL
    orderId required by /capi/v3/modifyTpSlOrder. That endpoint also expects a single
    triggerPrice, not separate TP and SL trigger fields. The previous implementation
    therefore generated HTTP 400 responses. We do not send that invalid request.
    """
    print("🛡️ V167.8.11 EXCHANGE TP/SL MODIFY SKIPPED: invalid use of market orderId")
    return {
        "success": False,
        "skipped": True,
        "reason": "INVALID_MARKET_ORDER_ID_FOR_MODIFY_TPSL",
        "symbol": symbol,
        "positionSide": position_side,
        "requested_tp": float(new_tp) if new_tp is not None else None,
        "requested_sl": float(new_sl) if new_sl is not None else None,
        "orderId": order_id,
    }
...
def place_demo_market_order(signal, candle, df, row):
    ...
    # V167.8.13: Minimum Risk Distance Guard.
    # The lower bound is the stricter of a fixed price percentage and an ATR-based
    # minimum, preventing very tight stops from turning normal noise into an exit.
    risk_distance_preflight = abs(entry_price - sl)
    min_risk_distance = max(
        entry_price * MIN_RISK_DISTANCE_PCT,
        float(atr) * MIN_RISK_DISTANCE_ATR_MULT,
    )
    if risk_distance_preflight < min_risk_distance:
        print()
        print("=" * 72)
        print("V167.8.13 MIN RISK DISTANCE GUARD")
        print("=" * 72)
        print(f"  risk_distance   : {risk_distance_preflight:.2f}")
        print(f"  min_required    : {min_risk_distance:.2f}")
        print(f"    pct component : {entry_price * MIN_RISK_DISTANCE_PCT:.2f} "
              f"({MIN_RISK_DISTANCE_PCT*100:.2f}% of {entry_price:.2f})")
        print(f"    atr component : {float(atr) * MIN_RISK_DISTANCE_ATR_MULT:.2f} "
              f"({MIN_RISK_DISTANCE_ATR_MULT:.2f} × ATR {float(atr):.2f})")
        print("🚫 ORDER BLOCKED: risk_distance below minimum")
        print("=" * 72)
        return None
    ...
    # V167.8.13: final pre-flight recheck against BOTH independent account caps.
    max_allowed_margin = available_balance * MAX_MARGIN_UTILIZATION
    max_allowed_notional = available_balance * MAX_NOTIONAL_PCT * MAX_LEVERAGE
    hard_cap_qty = min(
        (max_allowed_margin * MAX_LEVERAGE) / entry_price,
        max_allowed_notional / entry_price,
    )
    if qty > hard_cap_qty + 1e-12:
        safe_qty = np.floor(hard_cap_qty / QTY_STEP) * QTY_STEP
        safe_qty = float(round(safe_qty, 4))
        print(
            f"🛡️ V167.8.13 PRE-FLIGHT CAP RECHECK: qty={qty:.4f} > "
            f"hard_cap={hard_cap_qty:.4f} | shrinking → {safe_qty:.4f}"
        )
        if safe_qty < MIN_QTY:
            raise RuntimeError(
                f"Available {available_balance:.4f} SUSDT too small for "
                f"MIN_QTY={MIN_QTY} after V167.8.13 margin/notional caps."
            )
        qty = safe_qty
        actual_margin = (qty * entry_price) / MAX_LEVERAGE
        actual_notional = qty * entry_price

    if actual_margin > max_allowed_margin + 1e-9 or actual_notional > max_allowed_notional + 1e-9:
        raise RuntimeError(
            f"V167.8.13 sizing guard failure: margin={actual_margin:.4f}/{max_allowed_margin:.4f}, "
            f"notional={actual_notional:.4f}/{max_allowed_notional:.4f}"
        )
    ...
    if exchange_order_id:
        print(f"🔑 V167.8.11 EXCHANGE ORDER ID: {exchange_order_id}")
    else:
        print("⚠️ V167.8.11 EXCHANGE ORDER ID missing in response")
    ...
    try:
        ...
        if opened_pos is not None:
            ...
            if actual_entry and actual_entry > 0:
                ...
                actual_fsl, actual_ftp = calculate_exchange_failsafe_tp_sl(signal, actual_entry, actual_rd)
                if created_ms:
                ...
                _exit_log_event({
                    ...
                    "exchange_protection_mode": "ENTRY_ORDER_REQUESTED",
                    "exchange_protection_verification": "REQUESTED_UNVERIFIED",
                    "exchange_protection_truth": "UNVERIFIED",
                })
                print(f"🔧 POST-FILL RECONCILE: signal={entry_price:.2f} actual={actual_entry:.2f} "
                      f"slippage={actual_entry-entry_price:+.2f} risk_distance={actual_rd:.2f}")

                # V167.8.13: three-tier slippage guard.
                try:
                    slippage_abs = abs(float(actual_entry) - float(entry_price))
                    slippage_pct = slippage_abs / float(entry_price) if entry_price > 0 else 0.0
                    slippage_risk_ratio = (slippage_abs / float(actual_rd)) if actual_rd > 0 else 0.0
                    if slippage_pct <= SLIPPAGE_NORMAL_MAX_PCT:
                        slip_tier, slip_icon = "NORMAL", "✅"
                    elif slippage_pct <= SLIPPAGE_WARNING_MAX_PCT:
                        slip_tier, slip_icon = "WARNING", "🟡"
                    else:
                        slip_tier, slip_icon = "EXTREME", "🔴"

                    print(
                        f"{slip_icon} V167.8.13 SLIPPAGE GUARD [{slip_tier}]: "
                        f"abs={slippage_abs:.2f} pct={slippage_pct*100:.3f}% "
                        f"risk_ratio={slippage_risk_ratio*100:.1f}%"
                    )
                    _exit_log_event({
                        "event": "ENTRY_SLIPPAGE_GUARD",
                        "side": signal,
                        "signal_entry": float(entry_price),
                        "actual_entry": float(actual_entry),
                        "slippage_abs": float(slippage_abs),
                        "slippage_pct": float(slippage_pct),
                        "slippage_risk_ratio": float(slippage_risk_ratio),
                        "tier": slip_tier,
                    })

                    if slip_tier == "EXTREME" and slippage_risk_ratio >= SLIPPAGE_EXTREME_RISK_RATIO:
                        print(
                            f"🛑 V167.8.13 EXTREME SLIPPAGE — EMERGENCY EXIT "
                            f"({slippage_pct*100:.3f}% price, {slippage_risk_ratio*100:.1f}% of risk)"
                        )
                        _exit_log_event({
                            "event": "SLIPPAGE_EMERGENCY_EXIT",
                            "reason": "SLIPPAGE_EXIT",
                            "side": signal,
                            "signal_entry": float(entry_price),
                            "actual_entry": float(actual_entry),
                            "slippage_abs": float(slippage_abs),
                        ...
                        try:
                            opened_size = _position_size(opened_pos)
                            close_result, close_verified = _send_close_order(
                                opened_pos, opened_size, "SLIPPAGE_EXIT"
                            )
                            print(f"🛑 SLIPPAGE EMERGENCY EXIT: verified={close_verified}")
                            if not close_verified:
                                print("⚠️ SLIPPAGE EMERGENCY EXIT NOT VERIFIED; exit manager will reconcile safely")
                        except Exception as slip_exit_err:
                            print(f"⚠️ SLIPPAGE EMERGENCY EXIT FAILED: {slip_exit_err!r}")
                        return None
                    elif slip_tier == "WARNING":
                        print("🟡 V167.8.13 WARNING SLIPPAGE — trade continues; actual fill remains source of truth")
                except Exception as slip_err:
                    print(f"⚠️ V167.8.13 SLIPPAGE GUARD ERROR (ignoring): {slip_err!r}")

                print(
                    f"🛡️ V167.8.13 POST-FILL PROTECTION TRUTH: signalEntry={entry_price:.2f} actualFill={actual_entry:.2f} | "
                    f"requestedAtSignal SL={failsafe_sl:.1f} TP={failsafe_tp:.1f} | "
                    f"actualFillExpected SL={actual_fsl:.1f} TP={actual_ftp:.1f} | status=REQUESTED_UNVERIFIED"
                )
                _exit_log_event({
                    "event": "EXCHANGE_PROTECTION_TRUTH", "side": signal,
                    "signal_entry": float(entry_price), "actual_fill_entry": float(actual_entry),
                    "requested_exchange_sl": float(failsafe_sl), "requested_exchange_tp": float(failsafe_tp),
                    "actual_fill_expected_sl": float(actual_fsl), "actual_fill_expected_tp": float(actual_ftp),
                    "status": "REQUESTED_UNVERIFIED", "truth": "UNVERIFIED",
                })
        else:
            print("⚠️ V167.8.11: could not read newly-opened position for entry_atr snapshot")
    except Exception as e:
        print(f"⚠️ V167.8.11: entry_atr snapshot save failed: {repr(e)}")
...
_OPEN_ORDERS_NEG_CACHE = {"ts": 0.0, "reason": None}
_OPEN_ORDERS_NEG_TTL = 300.0


def get_open_demo_orders():
    return []
    last_err = None
    for sym in (DEMO_SYMBOL, MARKET_SYMBOL):
        try:
            payload = authenticated_get(
                EXCHANGE_OPEN_ORDERS_PATH,
                {"symbol": sym, "limit": 100, "page": 0},
            )
            rows = _parse_order_rows(payload)
            _OPEN_ORDERS_NEG_CACHE["reason"] = None
            _OPEN_ORDERS_NEG_CACHE["ts"] = 0.0
            return rows
        except Exception as e:
            last_err = repr(e)
            continue
    _OPEN_ORDERS_NEG_CACHE["ts"] = now
    _OPEN_ORDERS_NEG_CACHE["reason"] = last_err
    print(f"⚠️ V167.8.11 openOrders unsupported on demo ({last_err}); cache 5min")
    return []
...
def reconcile_exchange_protection(position, item):
    """V167.8.14: Demo does not support openOrders/openAlgoOrders.

    Protection truth is verified ONLY via /capi/v3/sim/order/history, which is
    the sole demo-supported order-inspection endpoint. Never invent protection:
    if nothing observable is found -> UNAVAILABLE.
    """
    if not EXCHANGE_PROTECTION_RECONCILIATION_ENABLED:
        item["exchange_protection_status"] = "UNAVAILABLE"
        item["exchange_protection_truth"] = "UNVERIFIED"
        return item
    try:
        _now = time.time()
        _cache = globals().setdefault("_PROT_CACHE", {"ts": 0.0, "orders": []})
        if _now - _cache["ts"] > 300.0:
            _cache["orders"] = get_recent_demo_orders(limit=ORDER_HISTORY_LOOKBACK_LIMIT)
            _cache["ts"] = _now
        ...
        found_tp = None
        for order in orders:
            if str(order.get("symbol", "")).upper() not in {DEMO_SYMBOL.upper(), MARKET_SYMBOL.upper()}:
                continue
            if _order_position_side(order) not in {"", side}:
                continue
            status = str(order.get("status") or order.get("orderStatus") or "").upper()
            if status not in {"NEW", "UNTRIGGERED", "PENDING", "PARTIALLY_FILLED", "PARTIAL_FILLED"}:
                continue
            tp_px = order.get("tpTriggerPrice") or order.get("tpPrice")
            sl_px = order.get("slTriggerPrice") or order.get("slPrice")
        ...
        tol = max(0.2, (expected_sl or expected_tp or 1.0) * 0.00002)
        sl_ok = expected_sl > 0 and found_sl is not None and abs(float(found_sl) - expected_sl) <= tol
        tp_ok = expected_tp > 0 and found_tp is not None and abs(float(found_tp) - expected_tp) <= tol
        if sl_ok and tp_ok:
            status, truth = "CONFIRMED", "CONFIRMED"
        elif found_sl is None and found_tp is None:
            status, truth = "UNAVAILABLE", "UNVERIFIED"
        else:
            status, truth = "MISMATCH", "UNVERIFIED"
        item["exchange_protection_status"] = status
        item["exchange_protection_truth"] = truth
        print(
            f"🛡️ V167.8.14 PROTECTION TRUTH (order/history): status={status} "
            f"expectedSL={expected_sl:.1f} observedSL={found_sl if found_sl is not None else 'NA'} "
            f"expectedTP={expected_tp:.1f} observedTP={found_tp if found_tp is not None else 'NA'}"
        )
        return item
    except Exception as e:
        item["exchange_protection_status"] = "UNAVAILABLE"
        ...
        print("⚠️ V167.8.14 EXCHANGE PROTECTION RECONCILIATION ERROR:", repr(e))
        return item
    try:
        orders = get_recent_demo_orders(limit=ORDER_HISTORY_LOOKBACK_LIMIT)
        side = _position_side(position)
        expected_sl = float(item.get("exchange_failsafe_sl", 0) or 0)
        expected_tp = float(item.get("exchange_failsafe_tp", 0) or 0)
        found_sl = None; found_tp = None
        for order in orders:
            if str(order.get("symbol", "")).upper() not in {DEMO_SYMBOL.upper(), MARKET_SYMBOL.upper()}:
                continue
            if _order_position_side(order) not in {"", side}:
                continue
            cid = _order_client_id(order).upper()
            plan = str(order.get("planType") or order.get("algoType") or order.get("type") or "").upper()
            px = _extract_trigger_price(order)
            if "STOP" in plan or "SL" in cid: found_sl = px or found_sl
            if "TAKE" in plan or "TP" in cid: found_tp = px or found_tp
        item["exchange_protection_last_check"] = pd.Timestamp.now(tz="UTC").isoformat()
        item["exchange_protection_sl_observed"] = found_sl
        item["exchange_protection_tp_observed"] = found_tp
        tol = max(0.2, (expected_sl or expected_tp or 1.0) * 0.00002)
        ...
        item["exchange_protection_status"] = status
        item["exchange_protection_truth"] = truth
        print(f"🛡️ V167.8.13 PROTECTION TRUTH: status={status} expectedSL={expected_sl:.1f} observedSL={found_sl if found_sl is not None else 'NA'} expectedTP={expected_tp:.1f} observedTP={found_tp if found_tp is not None else 'NA'}")
        return item
    except Exception as e:
        item["exchange_protection_status"] = "UNAVAILABLE"
        item["exchange_protection_truth"] = "UNVERIFIED"
        item["exchange_protection_error"] = repr(e)
        print("⚠️ V167.8.13 EXCHANGE PROTECTION RECONCILIATION ERROR:", repr(e))
        return item


def _set_post_fill_protection_metadata(item, signal, actual_entry, entry_atr):
    rd = EXIT_SL_ATR_MULT_BASE * float(entry_atr)
    failsafe_sl, failsafe_tp = calculate_exchange_failsafe_tp_sl(signal, actual_entry, rd)
    item["entry_price"] = float(actual_entry)
    item["entry_price_source"] = "EXCHANGE_POSITION"
    item["entry_atr"] = float(entry_atr)
    item["risk_distance"] = float(rd)
    item["hard_sl_price"] = round_price_step(
        actual_entry - rd if signal == "LONG" else actual_entry + rd, 0.1
    )
    item["exchange_failsafe_sl"] = float(failsafe_sl)
    item["exchange_failsafe_tp"] = float(failsafe_tp)
    item["exchange_protection_status"] = "REQUESTED_UNVERIFIED"
    item["exchange_protection_truth"] = "UNVERIFIED"
    item["post_fill_reconciled_at"] = pd.Timestamp.now(tz="UTC").isoformat()
    return item
...
def get_demo_last_price(force_refresh=False):
    ...
    for path, params, key in WEEX_LAST_PRICE_PATHS:
        bad_until = _LAST_PRICE_BAD.get(path, 0.0)
        if now < bad_until:
            continue
        t0 = time.time()
        try:
            ...
            if not r.ok:
                ...
                if 400 <= r.status_code < 500:
                    _LAST_PRICE_BAD[path] = now + 300.0
                continue
            data = r.json()
            if not isinstance(data, dict):
            ...
            if LAST_PRICE_TELEMETRY_ENABLED:
                ...
                if _LAST_PRICE_TELEMETRY["current_selected_path"] != path:
                    print(f"📡 V167.8.11 LAST_PRICE PATH CHANGED: now using '{path}' "
...
# ============================================================
# V167.8.11 — NEW: EFFECTIVE_OPENED_AT
# ============================================================
def _get_effective_opened_at(position, item=None):
    """V167.8.11: only trusted timestamps. Never fabricate age with now()."""
    if item:
...
# ============================================================
# RISK_DISTANCE SANITY CHECK + REBUILD (verified only)
# ============================================================
def _reconstruct_entry_atr_if_needed(item, position, df):
    ...
    if snap is not None:
        ...
        if snap_atr > 0 and snap_rd > 0:
            ...
            if state_rd <= 0 or state_rd < snap_rd * RISK_DISTANCE_SANITY_LOW or state_rd > snap_rd * RISK_DISTANCE_SANITY_HIGH:
                print(f"🔧 V167.8.11 SNAPSHOT-BASED REBUILD: state_rd={state_rd:.2f} "
                      f"snapshot_rd={snap_rd:.2f} → using snapshot")
    ...
    if history_atr is not None and history_atr > 0:
        ...
        state_rd = float(item.get("risk_distance", 0) or 0)
        if state_rd <= 0 or state_rd < history_rd * RISK_DISTANCE_SANITY_LOW or state_rd > history_rd * RISK_DISTANCE_SANITY_HIGH:
            print(f"🔧 V167.8.11 HISTORY-BASED REBUILD: state_rd={state_rd:.2f} "
    ...
    state_rd = float(item.get("risk_distance", 0) or 0)
    if state_rd <= 0:
        print("🚫 V167.8.11 ENTRY_ATR_UNKNOWN — SAFE HOLD")
...
def _ensure_exit_state_for_position(item, position, df):
    if item:
        item = _ensure_trade_ledger(item)
        ...
        if "thesis_invalid_confirm_count" not in item:
            item["thesis_invalid_confirm_count"] = 0
        if "thesis_invalid_last_confirm_ts" not in item:
            item["thesis_invalid_last_confirm_ts"] = None

        # V167.8.11: reconcile opened_at against live position.
        # This heals stale/corrupt/zero opened_at without losing a valid one.
        ...
        if old_ts is None or old_ts != effective_opened_at:
            ...
            print(f"🔧 V167.8.11 OPENED_AT RECONCILED: "
        ...
        # V167.8.13: explicit exchange-protection truth. Calculated expected
        # prices are never treated as proof that an exchange conditional order exists.
        entry_now = float(item.get("entry_price", 0) or 0)
        rd_now = float(item.get("risk_distance", 0) or 0)
        side_now = str(item.get("side", "")).upper()
        if entry_now > 0 and rd_now > 0 and side_now in {"LONG", "SHORT"}:
            exp_sl, exp_tp = calculate_exchange_failsafe_tp_sl(side_now, entry_now, rd_now)
            item.setdefault("exchange_requested_sl", item.get("exchange_failsafe_sl", exp_sl))
            item.setdefault("exchange_requested_tp", item.get("exchange_failsafe_tp", exp_tp))
            item["exchange_failsafe_sl"] = float(exp_sl)
            item["exchange_failsafe_tp"] = float(exp_tp)
            item.setdefault("exchange_protection_status", "REQUESTED_UNVERIFIED")
            item.setdefault("exchange_protection_truth", "UNVERIFIED")
        return item
    ...
    if created_ms is not None:
        ...
        if snap is not None:
            ...
            if snap_atr > 0 and snap_rd > 0:
                ...
                source = "SNAPSHOT"
                print(f"🔧 V167.8.11 FRESH STATE: using SNAPSHOT "
                      f"entry_atr={entry_atr:.2f} risk_distance={risk_distance:.2f}")
    ...
    if entry_atr is None or risk_distance is None:
        ...
        if hist_atr is not None and hist_atr > 0:
            ...
            print(f"🔧 V167.8.11 FRESH STATE: using HISTORY_REBUILD "
                  f"entry_atr={entry_atr:.2f} risk_distance={risk_distance:.2f}")

    if risk_distance is None or risk_distance <= 0:
        size = _position_size(position)
        hard_sl = None
        # V167.8.11: single source of truth for opened_at
        ...
        print("🚫 V167.8.11 FRESH STATE: ENTRY_ATR_UNKNOWN — SAFE HOLD")
        ...
        return {
            ...
            "initial_size": size,
            "exit_legs": [],
            "realized_closed_qty": 0.0,
            "aggregate_exit_price": None,
            "aggregate_final_r": None,
            "tp1_done": False,
            "tp2_done": False,
            "be_armed": False,
            ...
            "thesis_invalid_confirm_count": 0,
            "thesis_invalid_last_confirm_ts": None,
        }
    ...
    return {
        ...
        "exchange_requested_sl": float(calculate_exchange_failsafe_tp_sl(side, entry, risk_distance)[0]),
        "exchange_requested_tp": float(calculate_exchange_failsafe_tp_sl(side, entry, risk_distance)[1]),
        "exchange_failsafe_sl": float(calculate_exchange_failsafe_tp_sl(side, entry, risk_distance)[0]),
        "exchange_failsafe_tp": float(calculate_exchange_failsafe_tp_sl(side, entry, risk_distance)[1]),
        "exchange_protection_status": "REQUESTED_UNVERIFIED",
        "exchange_protection_truth": "UNVERIFIED",
        "thesis_status": "UNKNOWN",
        "thesis_valid": True,
        "thesis_invalidated_at": None,
...
def _ensure_trade_ledger(item):
    """V167.8.11: persistent per-fill exit ledger for accurate aggregate R accounting."""
    if not isinstance(item, dict):
        return item
    if not isinstance(item.get("exit_legs"), list):
        item["exit_legs"] = []
    item.setdefault("realized_closed_qty", 0.0)
    item.setdefault("aggregate_exit_price", None)
    item.setdefault("aggregate_final_r", None)
    return item


def _record_exit_fill(item, reason, qty, fill_price, order_id=None, filled_at=None):
    """Record one verified close fill exactly once and update aggregate trade result."""
    item = _ensure_trade_ledger(item)
    try:
        q = float(qty or 0.0)
        px = float(fill_price) if fill_price is not None else None
    except Exception:
        return item
    if q <= 0 or px is None or not np.isfinite(px) or px <= 0:
        return item

    oid = str(order_id) if order_id is not None else ""
    for leg in item["exit_legs"]:
        if oid and str(leg.get("order_id", "")) == oid:
            return item
    ...
    leg_r = None
    if entry > 0 and rd > 0:
        leg_r = ((px - entry) / rd) if side == "LONG" else ((entry - px) / rd)
    ...
    leg = {
        ...
        "filled_at": (_normalize_utc_timestamp(filled_at) or pd.Timestamp.now(tz="UTC")).isoformat(),
        "leg_r": round(float(leg_r), 6) if leg_r is not None else None,
    }
    item["exit_legs"].append(leg)

    initial_qty = float(item.get("initial_size", 0) or 0)
    total_qty = sum(float(x.get("qty", 0) or 0) for x in item["exit_legs"])
    item["realized_closed_qty"] = total_qty
    if total_qty > 0:
        item["aggregate_exit_price"] = sum(
            float(x.get("qty", 0) or 0) * float(x.get("fill_price", 0) or 0)
            for x in item["exit_legs"]
        ) / total_qty
    if entry > 0 and rd > 0 and total_qty > 0:
        signed_value = sum(
            float(x.get("qty", 0) or 0) * float(x.get("fill_price", 0) or 0)
            for x in item["exit_legs"]
        )
        avg_exit = signed_value / total_qty
        item["aggregate_final_r"] = (
            ((avg_exit - entry) / rd) if side == "LONG" else ((entry - avg_exit) / rd)
        ) * (total_qty / initial_qty) if initial_qty > 0 else None
    return item


def _aggregate_trade_r(item, fallback_exit_price=None, fallback_qty=None):
    """Return aggregate realized R across all verified close fills, not just the last leg."""
    item = _ensure_trade_ledger(item)
    if fallback_exit_price is not None:
        try:
            q = float(fallback_qty if fallback_qty is not None else item.get("initial_size", 0) or 0)
            _record_exit_fill(item, "FINAL_FALLBACK", q, fallback_exit_price)
        except Exception:
            pass
    return item.get("aggregate_final_r")
...
def _record_close_for_circuit_breaker(item, reason, position, closed_at=None, exit_price=None, exit_qty=None, order_id=None):
    """V167.8.11: aggregate all verified exit fills; final_r is whole-trade R."""
    item = _ensure_trade_ledger(item or {})
    ...
    if exit_price_f is not None and (exit_qty is not None or order_id is not None):
        _record_exit_fill(item, reason, exit_qty if exit_qty is not None else item.get("initial_size", 0),
                          exit_price_f, order_id=order_id, filled_at=closed_ts)
    elif exit_price_f is not None and not item.get("exit_legs"):
        _record_exit_fill(item, reason, item.get("initial_size", 0), exit_price_f,
                          order_id=order_id, filled_at=closed_ts)

    final_r = item.get("aggregate_final_r")
    try:
    ...
    side = str(item.get("side", "")).upper()
    entry = float(item.get("entry_price", 0) or 0)
    aggregate_exit = item.get("aggregate_exit_price")
    if aggregate_exit is not None:
        try:
            aggregate_exit = float(aggregate_exit)
        except Exception:
            aggregate_exit = None

    was_invalidated = bool(item.get("thesis_invalidated_at"))
    _exit_log_event({
        "event": "TRADE_CLOSED",
        "reason": reason,
        "side": side,
        ...
        "aggregate_exit_price": aggregate_exit,
        "final_r": round(final_r, 3) if final_r is not None else None,
        "final_leg_r": round(float(((exit_price_f-entry)/float(item.get("risk_distance",0))) if side == "LONG" else ((entry-exit_price_f)/float(item.get("risk_distance",0)))), 3) if exit_price_f and float(item.get("risk_distance",0) or 0) > 0 else None,
        "realized_closed_qty": item.get("realized_closed_qty", 0.0),
        "initial_size": item.get("initial_size"),
        "exit_legs": item.get("exit_legs", []),
        "closed_at": closed_ts.isoformat(),
        "opened_at": item.get("opened_at"),
        "opened_at_source": item.get("opened_at_source"),
        "tp1_done": bool(item.get("tp1_done", False)),
        "tp2_done": bool(item.get("tp2_done", False)),
        "tp_compressed": bool(item.get("tp_compressed", False)),
        "tp_compression_atr_ratio": item.get("tp_compression_atr_ratio"),
        "tp_compression_age_hours": item.get("tp_compression_age_hours"),
        "thesis_status": item.get("thesis_status"),
        "thesis_invalidated_at": item.get("thesis_invalidated_at"),
        "thesis_age_hours": item.get("thesis_age_hours"),
        "thesis_invalidated_r": item.get("thesis_invalidated_r"),
        "thesis_final_reason": reason if was_invalidated else None,
        "thesis_final_r": item.get("thesis_invalidated_r") if was_invalidated else None,
    })
    try:
        if final_r is not None:
            if final_r < 0 and reason in {"SL", "SL_EXTERNAL", "EXTERNAL_CLOSE"}:
                update_circuit_breaker(True, closed_ts)
                print(f"🧮 CIRCUIT BREAKER: loss recorded ({reason}) aggregate_R={final_r:.3f}")
            elif final_r > 0:
                update_circuit_breaker(False, closed_ts)
                print(f"🧮 CIRCUIT BREAKER: win recorded ({reason}) aggregate_R={final_r:.3f}")
            elif reason == "BE":
                print("🧮 CIRCUIT BREAKER: BE scratch (streak unchanged)")
        elif reason == "SL":
            update_circuit_breaker(True, closed_ts)
            print("🧮 CIRCUIT BREAKER: loss recorded (SL, R unavailable)")
    except Exception as e:
...
def manage_position_exits(df=None):
    ...
    _exit_mgr_started = time.monotonic()
    print(f"🫀 V167.8.13 EXIT HEARTBEAT: START t={time.strftime('%H:%M:%S')} UTC")
    try:
        positions = get_demo_positions(force_refresh=False)
        print(f"🫀 V167.8.13 EXIT HEARTBEAT: positions_fetch={time.monotonic()-_exit_mgr_started:.2f}s")
        active = [
        ...
        for k in list(state):
            if k not in active_keys:
                ...
                if close_order is not None:
                    ...
                    close_qty = _order_executed_qty(close_order)
                    _record_exit_fill(stale_item, close_reason, close_qty, close_price,
                                      order_id=_extract_order_id(close_order), filled_at=close_time)
                    _record_close_for_circuit_breaker(stale_item, close_reason, {}, close_time, close_price,
                                                      exit_qty=close_qty, order_id=_extract_order_id(close_order))
                else:
                    print(f"🛡️ EXTERNAL CLOSE RECONCILIATION: key={k} no matching fill found; keeping state for next successful sync")
                    continue
                state.pop(k, None)
        if not active:
        ...
        print(f"🫀 V167.8.13 EXIT HEARTBEAT: mark_fetch={time.monotonic()-_exit_mgr_started:.2f}s")
        last = None
        try:
        ...
        for live_position in active:
            ...
            if pending:
                ...
                if verified:
                    _record_exit_fill(item, pending_reason, reduction, filled_price, order_id=order_id)
                    ...
                    if pending_reason in {"TRAIL", "BE", "TIMEEXIT"}:
                        _record_close_for_circuit_breaker(
                            ...
                            exit_qty=reduction, order_id=order_id
                        )
                        state.pop(key, None)
                        _exit_save_state(state)
                        continue
                    ...
                    if position is None:
                        _record_close_for_circuit_breaker(
                            ...
                            exit_qty=reduction if reduction > 0 else item.get("initial_size", 0), order_id=order_id
                        )
                        state.pop(key, None)
                        _exit_save_state(state)
                        continue
            ...
            # ============================================================
            # V167.8.11 THESIS OBSERVATION (LOG ONLY)
            # ============================================================
            ...
            if THESIS_OBSERVATION_ENABLED and not item.get("tp1_done", False) and risk_distance > 0:
                # V167.8.11: use effective opened_at (STATE → POSITION → FALLBACK)
                ...
                if opened_at_src_obs != "STATE":
                    print(f"🔧 V167.8.11 THESIS_AGE source={opened_at_src_obs} "
                ...
                try:
                    ...
                    print(f"⚠️ V167.8.11 THESIS OBSERVATION error (ignoring, log only): {repr(e)}")
                    thesis_valid_obs = item.get("thesis_valid", True)
                ...
                if not thesis_valid_obs and item.get("thesis_invalidated_at") is None:
                    ...
                    print(
                        f"🔎 V167.8.13 THESIS INVALIDATED (observation + early-exit guard): "
                ...
                # ============================================================
                # V167.8.13 THESIS EARLY EXIT — confirmed INVALID only
                # ============================================================
                if THESIS_EARLY_EXIT_ENABLED and not item.get("tp1_done", False):
                    current_status = str(item.get("thesis_status", "UNKNOWN")).upper()
                    if current_status == "INVALID":
                        now_ts = pd.Timestamp.now(tz="UTC")
                        prev_confirm_ts = item.get("thesis_invalid_last_confirm_ts")
                        can_confirm = True
                        if prev_confirm_ts:
                            try:
                                prev_ts = _normalize_utc_timestamp(prev_confirm_ts)
                                if prev_ts is not None:
                                    can_confirm = (now_ts - prev_ts).total_seconds() >= THESIS_INVALID_CONFIRM_MIN_INTERVAL_SECONDS
                            except Exception:
                                can_confirm = True

                        if can_confirm:
                            count = int(item.get("thesis_invalid_confirm_count", 0) or 0) + 1
                            item["thesis_invalid_confirm_count"] = count
                            item["thesis_invalid_last_confirm_ts"] = now_ts.isoformat()
                            state[key] = item
                            _exit_save_state(state)

                        try:
                            age_minutes = float(age_hours_obs) * 60.0
                        except Exception:
                            age_minutes = None

                        if (age_minutes is not None
                                and age_minutes <= THESIS_EARLY_EXIT_WINDOW_MINUTES
                                and int(item.get("thesis_invalid_confirm_count", 0) or 0) >= THESIS_INVALID_CONFIRMATIONS_REQUIRED):
                            print(
                                f"🛑 V167.8.13 THESIS EARLY EXIT CONFIRMED: "
                                f"INVALID × {item.get('thesis_invalid_confirm_count')} "
                                f"at age={age_minutes:.2f}min "
                                f"(window={THESIS_EARLY_EXIT_WINDOW_MINUTES}min)"
                            )
                            _exit_log_event({
                                "event": "THESIS_EARLY_EXIT_CONFIRMED",
                                "reason": "THESIS_EARLY_EXIT",
                                "position_key": key,
                                "side": side,
                                "age_minutes": round(age_minutes, 2),
                                "confirmations": int(item.get("thesis_invalid_confirm_count", 0) or 0),
                                "current_r": round(current_r, 3),
                            })
                            try:
                                result, verified = _send_close_order(position, size, "THESIS_EXIT")
                            except Exception as thesis_exit_err:
                                result, verified = {"error": repr(thesis_exit_err)}, False
                                print(f"⚠️ V167.8.13 THESIS EARLY EXIT SEND ERROR: {thesis_exit_err!r}")

                            if verified:
                                thesis_fill = _extract_filled_price(result)
                                thesis_order_id = _extract_order_id(result)
                                if thesis_fill is not None:
                                    _record_exit_fill(
                                        item, "THESIS_EXIT", size, thesis_fill,
                                        order_id=thesis_order_id
                                    )
                                _record_close_for_circuit_breaker(
                                    item, "THESIS_EXIT", position,
                                    pd.Timestamp.now(tz="UTC"), thesis_fill,
                                    exit_qty=size, order_id=thesis_order_id
                                )
                                state.pop(key, None)
                                _exit_save_state(state)
                                continue
                                ...
                                _exit_save_state(state)
                                continue
                    elif current_status in {"VALID", "UNKNOWN"}:
                        if item.get("thesis_invalid_confirm_count", 0) != 0:
                            item["thesis_invalid_confirm_count"] = 0
                            item["thesis_invalid_last_confirm_ts"] = None
                            state[key] = item
                            _exit_save_state(state)
            ...
            # ============================================================
            # V167.8.11 DYNAMIC TP COMPRESSION — every iteration
            # ============================================================
            if (TP_COMPRESSION_ENABLED 
                ...
                if opened_at_src != "STATE" and opened_at is not None:
                    print(f"🔧 V167.8.11 TP_COMPRESSION_AGE source={opened_at_src} "
                ...
                if age_hours is not None and age_hours >= TP_COMPRESSION_MIN_AGE_HOURS and atr_ratio < TP_COMPRESSION_ATR_RATIO:
                    if thesis_valid:
                        ...
                        _exit_save_state(state)
                        print(
                            f"📉 V167.8.11 TP COMPRESSED: age={age_hours:.1f}h "
                        ...
                        _exit_save_state(state)
                        print(
                            f"⚠️ V167.8.11 TP COMPRESSION SKIPPED: age={age_hours:.1f}h "
                            f"atr_ratio={atr_ratio:.3f} thesis=INVALID"
                        )
                else:
                    # V167.8.13.1 CRITICAL FIX: unknown opened_at is a safe-hold
                    # condition, not a numeric age. Never compare/format None.
                    if age_hours is None:
                        reason = "AGE_UNKNOWN_SAFE_HOLD"
                    elif age_hours < TP_COMPRESSION_MIN_AGE_HOURS:
            ...
            if HARD_SL_WATCHDOG_ENABLED and hard_sl > 0 and sl_hit and size >= MIN_QTY:
                ...
                if verified:
                    ...
                    sl_fill = item["sl_fill_price"]
                    _record_exit_fill(item, "SL", size, sl_fill, order_id=_extract_order_id(result))
                    _record_close_for_circuit_breaker(
                        item, "SL", position, pd.Timestamp.now(tz="UTC"), sl_fill,
                        exit_qty=size, order_id=_extract_order_id(result)
                    )
                    state.pop(key, None)
                    _exit_save_state(state)
            ...
            if TIME_EXIT_ENABLED:
                ...
                if opened_at_src_te != "STATE" and opened_at_te is not None:
                    print(f"🔧 V167.8.11 TIME_EXIT_AGE source={opened_at_src_te} "
                ...
                if hours_open is not None and hours_open >= TIME_EXIT_HOURS and current_r >= TIME_EXIT_MIN_PROFIT_R:
                    ...
                    if verified:
                        timeexit_fill = _extract_filled_price(result)
                        _record_exit_fill(item, "TIMEEXIT", size, timeexit_fill, order_id=_extract_order_id(result))
                        _record_close_for_circuit_breaker(
                            item, "TIMEEXIT", position, pd.Timestamp.now(tz="UTC"), timeexit_fill,
                            exit_qty=size, order_id=_extract_order_id(result)
                        )
                        state.pop(key, None)
                    else:
            ...
            if not item.get("tp1_done", False) and current_r >= tp1_r_eff:
                ...
                if tp1_qty >= MIN_QTY:
                    ...
                    if verified:
                        tp1_fill = _extract_filled_price(result)
                        _record_exit_fill(item, "TP1", tp1_qty, tp1_fill, order_id=_extract_order_id(result))
                        _mark_exit_stage_done(item, "TP1", tp1_fill if tp1_fill is not None else mark, current_r)
                        item["pending_exit"] = None
                        state[key] = item
                        _exit_save_state(state)
                    else:
            ...
            if item.get("tp1_done", False) and not item.get("tp2_done", False) and current_r >= tp2_r_eff:
                ...
                if tp2_qty >= MIN_QTY:
                    ...
                    if verified:
                        tp2_fill = _extract_filled_price(result)
                        _record_exit_fill(item, "TP2", tp2_qty, tp2_fill, order_id=_extract_order_id(result))
                        _mark_exit_stage_done(item, "TP2", tp2_fill if tp2_fill is not None else mark, current_r)
                        item["pending_exit"] = None
                        state[key] = item
                        _exit_save_state(state)
                    else:
            ...
            if TRAILING_ENABLED and item.get("tp2_done", False):
                if not item.get("trailing_active", False) and current_r >= TRAILING_ACTIVATION_R:
                    item["trailing_active"] = True
                    print(f"🔄 TRAILING ACTIVATED @ {current_r:.2f}R")
            ...
            if item.get("be_armed", False):
                ...
                if be_hit and size >= MIN_QTY:
                    ...
                    if verified:
                        be_fill = _extract_filled_price(result)
                        _record_exit_fill(item, "BE", size, be_fill, order_id=_extract_order_id(result))
                        _record_close_for_circuit_breaker(
                            item, "BE", position, pd.Timestamp.now(tz="UTC"), be_fill,
                            exit_qty=size, order_id=_extract_order_id(result)
                        )
                        state.pop(key, None)
                    else:
        ...
        print(f"🚨 V167.8.13 EXIT MANAGER ERROR after {time.monotonic()-_exit_mgr_started:.2f}s: {repr(e)}")
        import traceback
        traceback.print_exc()
...
def main():
    try:
        ...
        print()
        print("=" * 72)
        print("V167.8.13 PROFESSIONAL BOT")
        print("  + EFFECTIVE_OPENED_AT (STATE → POSITION → FALLBACK)")
        print("  + CIRCUIT BREAKER TIMESTAMP FIX (root cause + defense)")
        print("  + OBSERVATION / TELEMETRY (thesis + last_price + trade_closed)")
        print("  + HISTORICAL ENTRY-ATR VERIFICATION")
        print("  + SAFE HOLD (no current-ATR fallback)")
        print("  + PERSISTENT ENTRY_ATR SNAPSHOT")
        print("  + RISK_DISTANCE SANITY CHECK + REBUILD")
        print("  + PER-ITERATION TP COMPRESSION RESTORE")
        print("  + EXIT STATE RESILIENCE")
        print("  + DYNAMIC TP COMPRESSION + THESIS VALIDITY GUARD")
        print("  + DUAL-PRICE HARD-SL + LAST_PRICE FAILSAFE")
        print("  + CONDITION-AWARE SESSION FILTER")
        print("  + FEE-AWARE BREAKEVEN + MARGIN BUFFER")
        print("=" * 72)
        ...
        print("V167.8.11 ADDITIONS : EFFECTIVE_OPENED_AT (single source of truth)")
        print("                     - Reconciliation in _ensure_exit_state_for_position")
        print("                     - Thesis Obs / TP Compression / TIME EXIT use it")
        print("V167.8.13 ADDITIONS : EXECUTION/EXIT SAFETY (4 GUARDS)")
        print("V167.8.13.1 PATCH   : TP COMPRESSION UNKNOWN-AGE SAFE HOLD")
        print("                     - MIN_RISK_DISTANCE (price % + ATR)")
        print("                     - 3 independent position/margin caps + post-rounding recheck")
        print("                     - 3-tier slippage guard + risk-distance ratio")
        print("                     - Confirmed thesis INVALID early exit")
        ...
        print("NOTIONAL CAP       :", f"MAX {MAX_NOTIONAL_PCT * 100:.0f}% × leverage of available balance")
        ...
        print("RISK_DIST REBUILD  : VERIFIED HISTORY ONLY (no CURRENT_ATR fallback)")
        ...
        print("THESIS OBSERVATION :", "ENABLED" if THESIS_OBSERVATION_ENABLED else "DISABLED")
        print("THESIS EARLY EXIT  :", "ENABLED" if THESIS_EARLY_EXIT_ENABLED else "DISABLED",
              f"(window={THESIS_EARLY_EXIT_WINDOW_MINUTES}m, confirmations={THESIS_INVALID_CONFIRMATIONS_REQUIRED})")
        print("MIN RISK DISTANCE  :", f"MAX({MIN_RISK_DISTANCE_PCT*100:.2f}% price, {MIN_RISK_DISTANCE_ATR_MULT:.2f}×ATR)")
        print("SLIPPAGE GUARD     :", f"NORMAL≤{SLIPPAGE_NORMAL_MAX_PCT*100:.2f}% | WARNING≤{SLIPPAGE_WARNING_MAX_PCT*100:.2f}% | EXTREME risk≥{SLIPPAGE_EXTREME_RISK_RATIO*100:.0f}%")
        print("LAST_PRICE TELEM.  :", "ENABLED (log-only)" if LAST_PRICE_TELEMETRY_ENABLED else "DISABLED")
        print("CIRCUIT BREAKER    : FIXED (timestamp + tz-normalization + legacy reset)")
        print("EFFECTIVE_OPENED_AT: ENABLED (STATE → POSITION → FALLBACK)")
        print("RUNNER             :", f"Trailing (activates after {TRAILING_ACTIVATION_R}R)")
        ...
        while True:
            try:
                ...
                try:
                    ...
                    print("🛡️ V167.8.11: NETWORK OUTAGE DETECTED — preserving exit state; no synthetic close prices will be created")
                    save_runtime_state(last_candle, status="NETWORK_DOWN")
                    network_wait("LIVE NETWORK RECOVERY")
                    print("🔄 V167.8.11: NETWORK RECOVERED — reconciling live position + exit ledger before new candle processing")
                    try:
                        manage_position_exits(df)
                    except Exception as reconcile_err:
                        print("⚠️ POST-RECOVERY EXIT RECONCILIATION ERROR:", repr(reconcile_err))
                    new_df, newest_closed = fetch_new_closed_candles(last_candle)
                ...
                print()
                print("=" * 72)
                print("V167.8.13 DEMO BOT STOPPED")
                print("=" * 72)
                break
            except Exception as e:
                print()
...
# NOTE: OFFLINE BUILD MUST NOT CALL main().
# main() performs credential/API startup checks and belongs to the live/demo runner.
# The offline entry point is the self-test block at the end of this file.

# ============================================================
# V167.8.13 OFFLINE SELF TEST
# ============================================================
...
def _offline_self_tests():
    ...
    try:
        # Minimum risk distance math.
        e = 76858.0
        atr = 153.8857
        rd = 230.8286
        min_rd = max(e * MIN_RISK_DISTANCE_PCT, atr * MIN_RISK_DISTANCE_ATR_MULT)
        check("Minimum risk distance guard math", rd < min_rd)
    except Exception:
        check("Minimum risk distance guard math", False)

    try:
        # Three-cap sizing: result must satisfy both margin and notional caps.
        q = calculate_position_size(76858.0, 76400.0, 18443.0, 0.02)
        margin = q * 76858.0 / MAX_LEVERAGE
        notional = q * 76858.0
        check("Three-cap sizing within margin cap", margin <= 18443.0 * MAX_MARGIN_UTILIZATION + 1e-6)
        check("Three-cap sizing within notional cap", notional <= 18443.0 * MAX_NOTIONAL_PCT * MAX_LEVERAGE + 1e-6)
    except Exception:
        check("Three-cap sizing", False)
    ...
    print("\n" + "=" * 72)
    print("V167.8.13 OFFLINE SELF TEST")
    print("=" * 72)
    passed = 0
...
if __name__ == "__main__":
    # DEFAULT = ONLINE DEMO RUNNER.
    # Set V167_OFFLINE_TEST=1 only when you explicitly want local self-tests.
    # This keeps credentials/API/demo execution intact while preserving a safe
    # offline test switch.
    if os.getenv("V167_OFFLINE_TEST", "0").strip().lower() in {"1", "true", "yes", "on"}:
        print("=" * 72)
        print("V167.8.13 OFFLINE SELF-TEST MODE")
    ...
    # main() still enforces DEMO_ONLY=True and validates credentials.
    main()
